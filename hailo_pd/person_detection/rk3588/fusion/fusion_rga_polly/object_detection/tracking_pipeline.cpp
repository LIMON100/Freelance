...
cv::Mat TrackingPipeline::assemble_patches(const std::vector<cv::Mat>& patches, std::vector<cv::Rect>& patch_locations_on_sheet) {
    if (patches.empty()) {
        return cv::Mat();
    }

    // Use the model input width as the fixed width of our "tile sheet"
    int sheet_width = m_config.model_input_width;
    int current_x = 0;
    int current_y = 0;
    int max_h_in_row = 0;

    patch_locations_on_sheet.clear();
    patch_locations_on_sheet.reserve(patches.size());

    // Simple greedy packing logic: place patches left-to-right, then move to the next row.
    for (const auto& patch : patches) {
        if (current_x + patch.cols > sheet_width) { // Move to next row
            current_y += max_h_in_row;
            current_x = 0;
            max_h_in_row = 0;
        }
        patch_locations_on_sheet.emplace_back(current_x, current_y, patch.cols, patch.rows);
        current_x += patch.cols;
        if (patch.rows > max_h_in_row) {
            max_h_in_row = patch.rows;
        }
    }

    // Create the final sheet and copy all the patches into their calculated locations.
    cv::Mat sheet = cv::Mat::zeros(current_y + max_h_in_row, sheet_width, patches[0].type());
    for (size_t i = 0; i < patches.size(); ++i) {
        patches[i].copyTo(sheet(patch_locations_on_sheet[i]));
    }
    return sheet;
}

/**
 * @brief Extracts unique (non-overlapping) regions from an image.
 * @param image The source image to extract from.
 * @param rois_to_subtract A vector of Rects defining the areas to consider "not unique" (i.e., overlapping).
 * @param patch_origins_in_image Output vector that stores the top-left Point of each extracted patch in the original image's coordinates.
 * @return A vector of cv::Mat patches corresponding to the unique regions.
 */
std::vector<cv::Mat> TrackingPipeline::extract_unique_patches(const cv::Mat& image, const std::vector<cv::Rect>& rois_to_subtract, std::vector<cv::Point>& patch_origins_in_image) {
    // Create a mask where everything is initially considered "unique" (white).
    cv::Mat unique_mask(image.size(), CV_8UC1, cv::Scalar(255));

    // Black out the regions that are covered by the reference camera's detections.
    for (const auto& roi : rois_to_subtract) {
        // We can inflate the ROI slightly to create a margin and avoid edge effects.
        cv::Rect inflated_roi = roi - cv::Point(10, 10) + cv::Size(20, 20);
        // Clamp the ROI to the image boundaries before drawing.
        inflated_roi &= cv::Rect(0, 0, image.cols, image.rows);
        cv::rectangle(unique_mask, inflated_roi, cv::Scalar(0), cv::FILLED);
    }
    
    // Find the contours of the remaining white areas. These are our unique regions.
    std::vector<std::vector<cv::Point>> contours;
    cv::findContours(unique_mask, contours, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);

    std::vector<cv::Mat> patches;
    patch_origins_in_image.clear();
    for (const auto& contour : contours) {
        cv::Rect bbox = cv::boundingRect(contour);
        // Only process patches that are reasonably large to avoid noise.
        if (bbox.area() > 1000) { 
            patches.push_back(image(bbox).clone());
            patch_origins_in_image.push_back(bbox.tl());
        }
    }
    return patches;
}
...
//     // Define the maximum time difference allowed between frames to be considered a "match".
//     const auto max_sync_delta = std::chrono::milliseconds(16); // ~ half a 30fps frame
...
//         // Step 2: Handle cases where one or both queues are temporarily empty.
//         if (!has_eo || !has_ir) {
//             // If either stream has permanently ended (stopped and empty), we can't make any more pairs, so exit the loop.
...
//         // Step 3: We have a frame from each queue. Compare their timestamps to decide what to do.
...
//             m_preprocessed_queue_ir->pop(ir_item); // Pop and discard the old IR frame.
...
//             m_preprocessed_queue_eo->pop(eo_item); // Pop and discard the old EO frame.
//             continue; // Loop immediately to compare the new EO frame against the same IR frame.
//         }
//         else {
//             // The frames are a good match! Now we officially pop them from the queues to be processed.
...
//         // If we reach this point, 'eo_item' and 'ir_item' contain a synchronized pair.
//         // Now, the rest of your original logic can execute.

//         // Use std::promise and std::future for synchronizing the two parallel inference calls.
...
//         // Wait here for both inference jobs to complete. This provides backpressure.
//         eo_future.wait();
//         ir_future.wait();

//         // Proceed with the fusion logic.
...
//             std::cerr << "[WARNING] Missing inference results for frame " << eo_item.frame_id << ". Skipping fusion." << std::endl;
...
//         std::vector<byte_track::Object> eo_objects = detections_to_bytetrack_objects(eo_bboxes, eo_item.org_frame.cols, eo_item.org_frame.rows);
...
void TrackingPipeline::fusion_and_inference_worker() {
    // Define the maximum time difference allowed between frames to be considered a "match".
    // 16ms is a good starting point (~half a 30fps frame duration).
    const auto max_sync_delta = std::chrono::milliseconds(16);

    while (!m_stop_flag.load()) {
        SingleStreamFrameData eo_item, ir_item;

        // --- TIMESTAMP-BASED FRAME SYNCHRONIZATION LOGIC ---
        ...
        if (time_diff > max_sync_delta) {
            ...
            // The frames are a good match! Now we can officially pop them from the queues to be processed.
            m_preprocessed_queue_eo->pop(eo_item);
            m_preprocessed_queue_ir->pop(ir_item);
        }
        ...
        // If we get here, 'eo_item' and 'ir_item' contain a synchronized pair.
        // Now, we can proceed with inference and fusion for this pair.
        ...
        // Wait here for both inference jobs to complete.
        eo_future.wait();
        ir_future.wait();
        ...
        // Get native EO detections and convert to full frame coords.
        ...
        // Get native IR detections and convert to full frame coords.
        ...
        if (m_config.enable_adaptive_alpha) {
            ...
            alpha = 0.7f; // Use the fixed default
        }
        auto t_alpha_end = std::chrono::high_resolution_clock::now();
        fused_item.alpha_calculation_duration = std::chrono::duration<double, std::milli>(t_alpha_end - t_alpha_start);

        for (auto& eo_obj : eo_objects) {
            cv::Point2f eo_center(eo_obj.rect.x() + eo_obj.rect.width() / 2.0f, eo_obj.rect.y() + eo_obj.rect.height() / 2.0f);
            cv::Point2f projected_ir_center = transformEoToIr(eo_center, current_zoom, current_focus);

            byte_track::Object* best_ir_match = nullptr;
            cv::Rect2f search_roi(projected_ir_center.x - search_roi_size / 2.0f, projected_ir_center.y - search_roi_size / 2.0f, search_roi_size, search_roi_size);

            for (auto& ir_obj : ir_objects) {
                cv::Point2f ir_center(ir_obj.rect.x() + ir_obj.rect.width() / 2.0f, ir_obj.rect.y() + ir_obj.rect.height() / 2.0f);
                if (search_roi.contains(ir_center)) {
                     best_ir_match = &ir_obj;
                     break;
                }
