...
std::vector<float> load_wav_file(const std::string& filepath, unsigned int expected_sample_rate) {
    ...
    if (sample_rate != expected_sample_rate) {
        ...
        throw std::runtime_error("Invalid sample rate for " + filepath + ". Expected " + std::to_string(expected_sample_rate) + " Hz.");
    }
...
int main(int argc, char* argv[]) {
    ...
    const std::string classifier_model_path = "../models/k3_150epoch.onnx";
    ...
    try {
        ...
        for (const auto& wav_path : wav_files) {
            ...
            try {
                ...
                // Display Result
                std::cout << "\n--- Prediction Result for " << wav_path << " ---" << std::endl;
