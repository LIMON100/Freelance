...
// *** Landmark Crop Scaling (EXPERIMENT HERE) ***
...
// --- Utility Functions ---
static inline float dequantize_int8_to_float(int8_t val, int32_t zp, float scale) { /* ... keep ... */ return ((float)val - (float)zp) * scale; }
static inline float dequantize_uint8_to_float(uint8_t val, int32_t zp, float scale) { /* ... keep ... */ return ((float)val - (float)zp) * scale; }
#ifdef __ARM_FP16_FORMAT_IEEE
    typedef __fp16 fp16_t;
    static inline float fp16_to_fp32(fp16_t x) { return (float)x; }
#else
    typedef unsigned short fp16_t;
    static float fp16_to_fp32(fp16_t x) { /* ... keep conversion ... */ int sign = (x >> 15) & 0x0001; int exponent = (x >> 10) & 0x001f; int mantissa = x & 0x03ff; float val; if (exponent == 0) { val = ldexpf((float)mantissa / 1024.0f, -14); } else if (exponent == 31) { val = (mantissa == 0) ? INFINITY : NAN; } else { val = ldexpf(1.0f + (float)mantissa / 1024.0f, exponent - 15); } return sign ? -val : val; }
#endif
static int clamp(int x, int min_val, int max_val) { /* ... keep ... */ return std::max(min_val, std::min(x, max_val)); }
static float clampf(float x, float min_val, float max_val) { /* ... keep ... */ return std::max(min_val, std::min(x, max_val)); }
static void dump_tensor_attr(rknn_tensor_attr *attr, const char* model_name) { /* ... keep ... */
    printf("%s Idx:%d Name:%s Dims:[%d,%d,%d,%d] Fmt:%s Type:%s(%d) Qnt:%s ZP:%d Scale:%.3f Size:%u Stride:%u\n",
       model_name, attr->index, attr->name,
       attr->n_dims > 0 ? attr->dims[0] : -1, attr->n_dims > 1 ? attr->dims[1] : -1,
       attr->n_dims > 2 ? attr->dims[2] : -1, attr->n_dims > 3 ? attr->dims[3] : -1,
       get_format_string(attr->fmt), get_type_string(attr->type), attr->type,
       get_qnt_type_string(attr->qnt_type), attr->zp, attr->scale,
       attr->size, attr->w_stride);
}
...
static int crop_image_simple(image_buffer_t *src_img, image_buffer_t *dst_img, box_rect_t crop_box) {
    ...
    if (src_img->format == IMAGE_FORMAT_RGB888) {
        ...
        // Handle formats other than RGB or BGR (if defined)
    ...
    if (crop_w <= 0 || crop_h <= 0) {
        printf("ERROR: crop ROI invalid size (%dx%d)\n", crop_w, crop_h);
        return -1;
    }
    int x_start = std::max(0, crop_x);
    int y_start = std::max(0, crop_y);
    ...
    if (valid_crop_w <= 0 || valid_crop_h <= 0) {
        printf("ERROR: Clamped crop ROI zero size.\n");
        return -1;
    }
...
static int calculate_letterbox_rects(int src_w, int src_h, int dst_w, int dst_h, int allow_slight_change, letterbox_t* out_letterbox, image_rect_t* out_dst_box) { /* ... keep ... */
    if (!out_letterbox || !out_dst_box || src_w <= 0 || src_h <= 0 || dst_w <= 0 || dst_h <= 0) { return -1; } float scale_w = (float)dst_w / src_w; float scale_h = (float)dst_h / src_h; float scale = std::min(scale_w, scale_h); int resize_w = (int)(src_w * scale); int resize_h = (int)(src_h * scale); if (allow_slight_change == 1) { if (resize_w % 2 != 0) resize_w--; if (resize_h % 2 != 0) resize_h--; if (resize_w <= 0 || resize_h <= 0) return -1; } int padding_w = dst_w - resize_w; int padding_h = dst_h - resize_h; out_dst_box->left = padding_w / 2; out_dst_box->top = padding_h / 2; if (out_dst_box->left % 2 != 0) out_dst_box->left--; if (out_dst_box->top % 2 != 0) out_dst_box->top--; out_dst_box->left = std::max(0, out_dst_box->left); out_dst_box->top = std::max(0, out_dst_box->top); out_dst_box->right = out_dst_box->left + resize_w; out_dst_box->bottom = out_dst_box->top + resize_h; out_dst_box->right = std::min(dst_w, out_dst_box->right); out_dst_box->bottom = std::min(dst_h, out_dst_box->bottom); out_letterbox->scale = scale; out_letterbox->x_pad = out_dst_box->left; out_letterbox->y_pad = out_dst_box->top; return 0;
}

// --- RetinaFace Helpers ---
typedef struct { box_rect_t norm_box; float score; int index; } RetinaFaceDecodedCandidate;
static bool compareRetinaCandidates(const RetinaFaceDecodedCandidate& a, const RetinaFaceDecodedCandidate& b) { /* ... keep ... */ return a.score > b.score; }
static float calculate_normalized_overlap(const box_rect_t& box1, const box_rect_t& box2) { /* ... keep ... */
    float xmin1 = box1.left / 10000.0f, ymin1 = box1.top / 10000.0f; float xmax1 = box1.right / 10000.0f, ymax1 = box1.bottom / 10000.0f; float xmin2 = box2.left / 10000.0f, ymin2 = box2.top / 10000.0f; float xmax2 = box2.right / 10000.0f, ymax2 = box2.bottom / 10000.0f; float inter_xmin = std::max(xmin1, xmin2); float inter_ymin = std::max(ymin1, ymin2); float inter_xmax = std::min(xmax1, xmax2); float inter_ymax = std::min(ymax1, ymax2); float inter_w = std::max(0.0f, inter_xmax - inter_xmin); float inter_h = std::max(0.0f, inter_ymax - inter_ymin); float inter_area = inter_w * inter_h; float area1 = (xmax1 - xmin1) * (ymax1 - ymin1); float area2 = (xmax2 - xmin2) * (ymax2 - ymin2); float union_area = area1 + area2 - inter_area; return (union_area <= 1e-6f) ? 0.0f : (inter_area / union_area);
}
static void nms_retina(std::vector<RetinaFaceDecodedCandidate>& candidates, float nms_threshold) { /* ... keep ... */
    if (candidates.empty()) return; std::sort(candidates.begin(), candidates.end(), compareRetinaCandidates); std::vector<bool> removed(candidates.size(), false); std::vector<RetinaFaceDecodedCandidate> result_candidates; result_candidates.reserve(candidates.size()); for (size_t i = 0; i < candidates.size(); ++i) { if (removed[i]) continue; result_candidates.push_back(candidates[i]); for (size_t j = i + 1; j < candidates.size(); ++j) { if (removed[j]) continue; float overlap = calculate_normalized_overlap(candidates[i].norm_box, candidates[j].norm_box); if (overlap > nms_threshold) { removed[j] = true; } } } candidates = std::move(result_candidates);
}
...
static int post_process_retinaface_detection_only(
    ...
    face_analyzer_result_t* out_result) {
    // ... (Keep implementation from previous corrected version) ...
    out_result->count = 0; if (!app_ctx || !output_mems || !out_result || app_ctx->io_num_detect.n_output < 3) { printf("Retina PP: Invalid args or output count < 3.\n"); return -1; } rknn_tensor_attr* loc_attr = &app_ctx->output_attrs_detect[0]; rknn_tensor_attr* score_attr = &app_ctx->output_attrs_detect[1]; void* loc_buf = output_mems[0]->virt_addr; void* score_buf = output_mems[1]->virt_addr; if (!loc_buf || !score_buf) { printf("Retina PP: Output buffer is NULL.\n"); return -1; } const float (*prior_ptr)[4] = nullptr; int num_priors = 0; if (app_ctx->model1_height == 320) { num_priors = 4200; prior_ptr = BOX_PRIORS_320; } else if (app_ctx->model1_height == 640) { num_priors = 16800; prior_ptr = BOX_PRIORS_640; } else { printf("Retina PP: Unsupported model height %d.\n", app_ctx->model1_height); return -1; } bool is_quant = app_ctx->is_quant_detect; std::vector<RetinaFaceDecodedCandidate> decoded_candidates; decoded_candidates.reserve(num_priors / 10); const float VARIANCES[2] = {0.1f, 0.2f};
    for (int i = 0; i < num_priors; ++i) { float face_score; if (score_attr->type == RKNN_TENSOR_FLOAT16) { face_score = fp16_to_fp32(((fp16_t*)score_buf)[i * 2 + 1]); } else if (is_quant) { if (score_attr->type == RKNN_TENSOR_INT8) { face_score = dequantize_int8_to_float(((int8_t*)score_buf)[i * 2 + 1], score_attr->zp, score_attr->scale); } else if (score_attr->type == RKNN_TENSOR_UINT8) { face_score = dequantize_uint8_to_float(((uint8_t*)score_buf)[i * 2 + 1], score_attr->zp, score_attr->scale); } else { face_score = ((float*)score_buf)[i * 2 + 1]; } } else { face_score = ((float*)score_buf)[i * 2 + 1]; }
        if (face_score >= RETINA_CONF_THRESHOLD) { RetinaFaceDecodedCandidate cand; cand.score = face_score; cand.index = i; float dx, dy, dw, dh; if (loc_attr->type == RKNN_TENSOR_FLOAT16) { dx = fp16_to_fp32(((fp16_t*)loc_buf)[i * 4 + 0]); dy = fp16_to_fp32(((fp16_t*)loc_buf)[i * 4 + 1]); dw = fp16_to_fp32(((fp16_t*)loc_buf)[i * 4 + 2]); dh = fp16_to_fp32(((fp16_t*)loc_buf)[i * 4 + 3]); } else if (is_quant) { if (loc_attr->type == RKNN_TENSOR_INT8) { dx = dequantize_int8_to_float(((int8_t*)loc_buf)[i * 4 + 0], loc_attr->zp, loc_attr->scale); dy = dequantize_int8_to_float(((int8_t*)loc_buf)[i * 4 + 1], loc_attr->zp, loc_attr->scale); dw = dequantize_int8_to_float(((int8_t*)loc_buf)[i * 4 + 2], loc_attr->zp, loc_attr->scale); dh = dequantize_int8_to_float(((int8_t*)loc_buf)[i * 4 + 3], loc_attr->zp, loc_attr->scale); } else if (loc_attr->type == RKNN_TENSOR_UINT8) { dx = dequantize_uint8_to_float(((uint8_t*)loc_buf)[i * 4 + 0], loc_attr->zp, loc_attr->scale); dy = dequantize_uint8_to_float(((uint8_t*)loc_buf)[i * 4 + 1], loc_attr->zp, loc_attr->scale); dw = dequantize_uint8_to_float(((uint8_t*)loc_buf)[i * 4 + 2], loc_attr->zp, loc_attr->scale); dh = dequantize_uint8_to_float(((uint8_t*)loc_buf)[i * 4 + 3], loc_attr->zp, loc_attr->scale); } else { dx = ((float*)loc_buf)[i * 4 + 0]; dy = ((float*)loc_buf)[i * 4 + 1]; dw = ((float*)loc_buf)[i * 4 + 2]; dh = ((float*)loc_buf)[i * 4 + 3]; } } else { dx = ((float*)loc_buf)[i * 4 + 0]; dy = ((float*)loc_buf)[i * 4 + 1]; dw = ((float*)loc_buf)[i * 4 + 2]; dh = ((float*)loc_buf)[i * 4 + 3]; }
            float prior_cx=prior_ptr[i][0]; float prior_cy=prior_ptr[i][1]; float prior_w=prior_ptr[i][2]; float prior_h=prior_ptr[i][3]; float decoded_cx=dx*VARIANCES[0]*prior_w+prior_cx; float decoded_cy=dy*VARIANCES[0]*prior_h+prior_cy; float decoded_w=expf(dw*VARIANCES[1])*prior_w; float decoded_h=expf(dh*VARIANCES[1])*prior_h; float xmin_norm=decoded_cx-decoded_w*0.5f; float ymin_norm=decoded_cy-decoded_h*0.5f; float xmax_norm=decoded_cx+decoded_w*0.5f; float ymax_norm=decoded_cy+decoded_h*0.5f; cand.norm_box.left=(int)(clampf(xmin_norm,0.0f,1.0f)*10000.0f); cand.norm_box.top=(int)(clampf(ymin_norm,0.0f,1.0f)*10000.0f); cand.norm_box.right=(int)(clampf(xmax_norm,0.0f,1.0f)*10000.0f); cand.norm_box.bottom=(int)(clampf(ymax_norm,0.0f,1.0f)*10000.0f); decoded_candidates.push_back(cand); } }
    nms_retina(decoded_candidates, RETINA_NMS_THRESHOLD); float model_in_w=(float)app_ctx->model1_width; float model_in_h=(float)app_ctx->model1_height;
    for (const auto& cand : decoded_candidates) { if (cand.score < RETINA_VIS_THRESHOLD) { continue; } float xmin_norm=cand.norm_box.left/10000.0f; float ymin_norm=cand.norm_box.top/10000.0f; float xmax_norm=cand.norm_box.right/10000.0f; float ymax_norm=cand.norm_box.bottom/10000.0f; float xmin_model=xmin_norm*model_in_w; float ymin_model=ymin_norm*model_in_h; float xmax_model=xmax_norm*model_in_w; float ymax_model=ymax_norm*model_in_h; float xmin_unpad=(xmin_model-det_letter_box->x_pad)/det_letter_box->scale; float ymin_unpad=(ymin_model-det_letter_box->y_pad)/det_letter_box->scale; float xmax_unpad=(xmax_model-det_letter_box->x_pad)/det_letter_box->scale; float ymax_unpad=(ymax_model-det_letter_box->y_pad)/det_letter_box->scale;
        if (out_result->count < MAX_FACE_RESULTS) { face_object_t* final_face = &out_result->faces[out_result->count]; memset(final_face, 0, sizeof(face_object_t)); final_face->box.left=clamp((int)roundf(xmin_unpad),0,src_img_width-1); final_face->box.top=clamp((int)roundf(ymin_unpad),0,src_img_height-1); final_face->box.right=clamp((int)roundf(xmax_unpad),0,src_img_width-1); final_face->box.bottom=clamp((int)roundf(ymax_unpad),0,src_img_height-1); final_face->score=cand.score; if (final_face->box.right > final_face->box.left && final_face->box.bottom > final_face->box.top) { out_result->count++; } } else { printf("WARN: Exceeded MAX_FACE_RESULTS (%d)\n", MAX_FACE_RESULTS); break; } }
    return 0;
}
...
static int post_process_face_landmarks(
    ...
    point_t out_landmarks[NUM_FACE_LANDMARKS]) {
    // ... (Keep implementation from previous corrected version) ...
    rknn_tensor_attr* landmark_attr = &app_ctx->output_attrs_landmark[0]; if (landmark_attr->type != RKNN_TENSOR_INT8) { printf("LmkPP: Wrong output type %d\n", landmark_attr->type); return -1; } bool valid_dims = false; int num_elements = 0; if (landmark_attr->n_dims >= 2) { num_elements = 1; for(uint32_t d=0; d<landmark_attr->n_dims; ++d) num_elements *= landmark_attr->dims[d]; valid_dims = (num_elements == NUM_FACE_LANDMARKS * 3); } if (!valid_dims) { printf("LmkPP: Invalid dims/elems (%d elems)\n", num_elements); return -1; } int8_t* landmark_data = (int8_t*)landmark_mem->virt_addr; if (!landmark_data) { printf("LmkPP: Output virt_addr NULL\n"); return -1;} float landmark_scale = landmark_attr->scale; int32_t landmark_zp = landmark_attr->zp; float crop_roi_w = (float)(crop_roi.right - crop_roi.left); float crop_roi_h = (float)(crop_roi.bottom - crop_roi.top); if (crop_roi_w <= 0 || crop_roi_h <= 0) { printf("LmkPP: Invalid crop ROI %fx%f\n", crop_roi_w, crop_roi_h); return -1; }
    for (int i = 0; i < NUM_FACE_LANDMARKS; ++i) { int offset = i * 3; if (offset + 2 >= num_elements) { printf("LmkPP: Index out of bounds %d\n", offset+2); return -1;} float x_lmk_model = dequantize_int8_to_float(landmark_data[offset + 0], landmark_zp, landmark_scale); float y_lmk_model = dequantize_int8_to_float(landmark_data[offset + 1], landmark_zp, landmark_scale); float x_unpadded = (x_lmk_model - lmk_letter_box->x_pad); float y_unpadded = (y_lmk_model - lmk_letter_box->y_pad); float x_orig_crop_rel = x_unpadded / lmk_letter_box->scale; float y_orig_crop_rel = y_unpadded / lmk_letter_box->scale; float final_x = x_orig_crop_rel + crop_roi.left; float final_y = y_orig_crop_rel + crop_roi.top; out_landmarks[i].x = (int)roundf(final_x); out_landmarks[i].y = (int)roundf(final_y); } return 0;
}
...
static int post_process_iris_landmarks(
    ...
    point_t out_eye_contour[NUM_EYE_CONTOUR_LANDMARKS], point_t out_iris[NUM_IRIS_LANDMARKS]) {
    // ... (Keep implementation from previous corrected version) ...
    if (app_ctx->io_num_iris.n_output < 2) { return -1; } rknn_tensor_attr* eye_contour_attr = &app_ctx->output_attrs_iris[0]; rknn_tensor_attr* iris_pts_attr = &app_ctx->output_attrs_iris[1]; bool eye_dims_ok = (eye_contour_attr->n_elems == NUM_EYE_CONTOUR_LANDMARKS * 3); bool iris_dims_ok = (iris_pts_attr->n_elems == NUM_IRIS_LANDMARKS * 3); if (!eye_dims_ok || !iris_dims_ok || eye_contour_attr->type != RKNN_TENSOR_INT8 || iris_pts_attr->type != RKNN_TENSOR_INT8) { return -1; } int8_t* eye_data = (int8_t*)iris_output_mems[0]->virt_addr; if (!eye_data) { return -1; } float eye_scale = eye_contour_attr->scale; int32_t eye_zp = eye_contour_attr->zp; int8_t* iris_data = (int8_t*)iris_output_mems[1]->virt_addr; if (!iris_data) { return -1; } float iris_scale = iris_pts_attr->scale; int32_t iris_zp = iris_pts_attr->zp; float eye_crop_roi_w = (float)(eye_crop_roi.right - eye_crop_roi.left); float eye_crop_roi_h = (float)(eye_crop_roi.bottom - eye_crop_roi.top); if (eye_crop_roi_w <= 0 || eye_crop_roi_h <= 0) { return -1; }
    for (int i = 0; i < NUM_EYE_CONTOUR_LANDMARKS; ++i) { int offset = i * 3; if (offset + 2 >= eye_contour_attr->n_elems) { return -1;} float x_eye_model = dequantize_int8_to_float(eye_data[offset + 0], eye_zp, eye_scale); float y_eye_model = dequantize_int8_to_float(eye_data[offset + 1], eye_zp, eye_scale); float x_unpadded = (x_eye_model - iris_letter_box->x_pad); float y_unpadded = (y_eye_model - iris_letter_box->y_pad); float x_orig_eye_rel = x_unpadded / iris_letter_box->scale; float y_orig_eye_rel = y_unpadded / iris_letter_box->scale; float final_x = x_orig_eye_rel + eye_crop_roi.left; float final_y = y_orig_eye_rel + eye_crop_roi.top; out_eye_contour[i].x = (int)roundf(final_x); out_eye_contour[i].y = (int)roundf(final_y); }
    for (int i = 0; i < NUM_IRIS_LANDMARKS; ++i) { int offset = i * 3; if (offset + 2 >= iris_pts_attr->n_elems) { return -1;} float x_iris_model = dequantize_int8_to_float(iris_data[offset + 0], iris_zp, iris_scale); float y_iris_model = dequantize_int8_to_float(iris_data[offset + 1], iris_zp, iris_scale); float x_unpadded = (x_iris_model - iris_letter_box->x_pad); float y_unpadded = (y_iris_model - iris_letter_box->y_pad); float x_orig_eye_rel = x_unpadded / iris_letter_box->scale; float y_orig_eye_rel = y_unpadded / iris_letter_box->scale; float final_x = x_orig_eye_rel + eye_crop_roi.left; float final_y = y_orig_eye_rel + eye_crop_roi.top; out_iris[i].x = (int)roundf(final_x); out_iris[i].y = (int)roundf(final_y); } return 0;
}
...
int init_face_analyzer(const char *retinaface_model_path, const char* landmark_model_path, const char* iris_model_path, face_analyzer_app_context_t *app_ctx) {
    // ... (Keep implementation from previous corrected version) ...
    memset(app_ctx, 0, sizeof(face_analyzer_app_context_t)); int ret; int model_len = 0; unsigned char *model_buf = NULL;
    printf("Init Face Detect Model (RetinaFace): %s\n", retinaface_model_path); model_len = read_data_from_file(retinaface_model_path, reinterpret_cast<char**>(&model_buf)); if (!model_buf) { return -1; } ret = rknn_init(&app_ctx->rknn_ctx_detect, model_buf, model_len, 0, NULL); free(model_buf); model_buf = NULL; if (ret < 0) { return -1; } ret = rknn_set_core_mask(app_ctx->rknn_ctx_detect, RKNN_NPU_CORE_0); if (ret < 0) { printf("WARN: set core mask failed\n");} else { printf("INFO: Detect assigned Core 0.\n"); } ret = rknn_query(app_ctx->rknn_ctx_detect, RKNN_QUERY_IN_OUT_NUM, &app_ctx->io_num_detect, sizeof(app_ctx->io_num_detect)); if (ret != RKNN_SUCC || app_ctx->io_num_detect.n_output < 3) { printf("ERROR: RetinaFace model needs at least 3 outputs (loc, score, landms).\n"); goto cleanup_detect; }
    app_ctx->input_attrs_detect = (rknn_tensor_attr*)malloc(app_ctx->io_num_detect.n_input * sizeof(rknn_tensor_attr)); if (!app_ctx->input_attrs_detect) { ret = -1; goto cleanup_detect; } memset(app_ctx->input_attrs_detect, 0, app_ctx->io_num_detect.n_input * sizeof(rknn_tensor_attr)); for (uint32_t i = 0; i < app_ctx->io_num_detect.n_input; i++) { app_ctx->input_attrs_detect[i].index = i; ret = rknn_query(app_ctx->rknn_ctx_detect, RKNN_QUERY_NATIVE_INPUT_ATTR, &app_ctx->input_attrs_detect[i], sizeof(rknn_tensor_attr)); if (ret != RKNN_SUCC) goto cleanup_detect; dump_tensor_attr(&app_ctx->input_attrs_detect[i], "DetectIn(Native)"); }
    app_ctx->output_attrs_detect = (rknn_tensor_attr*)malloc(app_ctx->io_num_detect.n_output * sizeof(rknn_tensor_attr)); if (!app_ctx->output_attrs_detect) { ret = -1; goto cleanup_detect; } memset(app_ctx->output_attrs_detect, 0, app_ctx->io_num_detect.n_output * sizeof(rknn_tensor_attr)); for (uint32_t i = 0; i < app_ctx->io_num_detect.n_output; i++) { app_ctx->output_attrs_detect[i].index = i; ret = rknn_query(app_ctx->rknn_ctx_detect, RKNN_QUERY_NATIVE_NHWC_OUTPUT_ATTR, &app_ctx->output_attrs_detect[i], sizeof(rknn_tensor_attr)); if (ret != RKNN_SUCC) goto cleanup_detect; dump_tensor_attr(&app_ctx->output_attrs_detect[i], "DetectOut(Native)"); }
    if (app_ctx->io_num_detect.n_input > 0) { app_ctx->input_attrs_detect[0].type = RKNN_TENSOR_UINT8; app_ctx->input_attrs_detect[0].fmt = RKNN_TENSOR_NHWC; app_ctx->input_fmt_detect = RKNN_TENSOR_NHWC; }
...
// --- inference_face_analyzer (Updated for RetinaFace and Clamping Fix) ---
int inference_face_analyzer(face_analyzer_app_context_t *app_ctx,
                            image_buffer_t *src_img,
                            face_analyzer_result_t *out_result) {
    int ret = 0;
    ...
    // --- Stage 1: Face Detection (RetinaFace) ---
    // (Preprocessing, Run, Postprocessing - keep as is)
    // ... (Code for detect preproc, run, sync, post_process_retinaface_detection_only) ...
    if (!app_ctx || !app_ctx->input_mems_detect || !app_ctx->input_mems_detect[0] || !app_ctx->input_mems_detect[0]->virt_addr) { printf("ERROR: Detect input mem null\n"); return -1; } image_buffer_t detect_input_buf_wrapper; /* setup */ detect_input_buf_wrapper.width = app_ctx->model1_width; detect_input_buf_wrapper.height = app_ctx->model1_height; detect_input_buf_wrapper.format = IMAGE_FORMAT_RGB888; detect_input_buf_wrapper.virt_addr = (unsigned char*)app_ctx->input_mems_detect[0]->virt_addr; detect_input_buf_wrapper.size = app_ctx->input_mems_detect[0]->size; detect_input_buf_wrapper.fd = app_ctx->input_mems_detect[0]->fd; detect_input_buf_wrapper.width_stride = app_ctx->input_attrs_detect[0].w_stride; detect_input_buf_wrapper.height_stride = app_ctx->input_attrs_detect[0].h_stride ? app_ctx->input_attrs_detect[0].h_stride : app_ctx->model1_height;
    ret = convert_image_with_letterbox(src_img, &detect_input_buf_wrapper, &det_letter_box, bg_color); if (ret < 0) { printf("ERROR: convert_image_with_letterbox (detect) failed! ret=%d\n", ret); return ret; }
    #ifdef ZERO_COPY
    ret = rknn_mem_sync(app_ctx->rknn_ctx_detect, app_ctx->input_mems_detect[0], RKNN_MEMORY_SYNC_TO_DEVICE); if (ret < 0) { printf("ERROR: sync detect input failed! ret=%d\n", ret); return ret; }
    #endif
    // *** SET IO MEM for Detect Input ***
    ret = rknn_set_io_mem(app_ctx->rknn_ctx_detect, app_ctx->input_mems_detect[0], &app_ctx->input_attrs_detect[0]); if (ret < 0) { printf("ERROR: set io mem detect input failed! ret=%d\n", ret); return ret;}

    ret = rknn_run(app_ctx->rknn_ctx_detect, nullptr); if (ret < 0) { printf("ERROR: rknn_run (detect) failed! ret=%d\n", ret); return ret; }
    #ifdef ZERO_COPY
    for (uint32_t i = 0; i < app_ctx->io_num_detect.n_output; ++i) { ret = rknn_mem_sync(app_ctx->rknn_ctx_detect, app_ctx->output_mems_detect[i], RKNN_MEMORY_SYNC_FROM_DEVICE); if (ret < 0) { printf("ERROR: sync detect output %u failed! ret=%d\n", i, ret); return ret; } }
    #endif
    ret = post_process_retinaface_detection_only(app_ctx->output_mems_detect, app_ctx, &det_letter_box, src_w, src_h, out_result); if (ret < 0) { printf("ERROR: post_process_retinaface failed! ret=%d\n", ret); return ret; }

    int num_faces_to_process = out_result->count;
    if (num_faces_to_process == 0) return 0; // No faces found

    // --- Setup Wrappers for Landmark/Iris NPU Input Buffers ---
    // ... (Setup landmark_input_buf_wrapper and iris_input_buf_wrapper as before) ...
    image_buffer_t landmark_input_buf_wrapper; /* setup */ landmark_input_buf_wrapper.width=app_ctx->model2_width; landmark_input_buf_wrapper.height=app_ctx->model2_height; landmark_input_buf_wrapper.format=IMAGE_FORMAT_RGB888; landmark_input_buf_wrapper.virt_addr=(unsigned char*)app_ctx->input_mems_landmark[0]->virt_addr; landmark_input_buf_wrapper.size=app_ctx->input_mems_landmark[0]->size; landmark_input_buf_wrapper.fd=app_ctx->input_mems_landmark[0]->fd; landmark_input_buf_wrapper.width_stride=app_ctx->input_attrs_landmark[0].w_stride; landmark_input_buf_wrapper.height_stride=app_ctx->input_attrs_landmark[0].h_stride ? app_ctx->input_attrs_landmark[0].h_stride : app_ctx->model2_height;
    image_buffer_t iris_input_buf_wrapper; /* setup */ iris_input_buf_wrapper.width=app_ctx->model3_width; iris_input_buf_wrapper.height=app_ctx->model3_height; iris_input_buf_wrapper.format=IMAGE_FORMAT_RGB888; iris_input_buf_wrapper.virt_addr=(unsigned char*)app_ctx->input_mems_iris[0]->virt_addr; iris_input_buf_wrapper.size=app_ctx->input_mems_iris[0]->size; iris_input_buf_wrapper.fd=app_ctx->input_mems_iris[0]->fd; iris_input_buf_wrapper.width_stride=app_ctx->input_attrs_iris[0].w_stride; iris_input_buf_wrapper.height_stride=app_ctx->input_attrs_iris[0].h_stride ? app_ctx->input_attrs_iris[0].h_stride : app_ctx->model3_height;
    ...
    for (int i = 0; i < num_faces_to_process; ++i) {
        ...
        // --- Stage 2: Face Landmarks ---
        letterbox_t lmk_letter_box;
        image_rect_t src_rect_lmk;          // *Desired* source rect from original image
        ...
        // Calculate geometric center of the RetinaFace box
        int box_center_x = (current_face_result->box.left + current_face_result->box.right) / 2;
        int box_center_y = (current_face_result->box.top + current_face_result->box.bottom) / 2;
        int box_w = current_face_result->box.right - current_face_result->box.left;
        int box_h = current_face_result->box.bottom - current_face_result->box.top;

        // **** ADJUSTMENT: Calculate the crop center ****
        // Start with the geometric center.
        int crop_center_x = box_center_x;
        int crop_center_y = box_center_y;

        // ** Introduce Vertical Shift **
        // Shift the center *down* relative to the RetinaFace box height.
        // Experiment with the multiplier (e.g., 0.05 to 0.15). Start around 0.10.
        int vertical_shift = static_cast<int>(box_h * 0.10f);
        crop_center_y = crop_center_y + vertical_shift;
        // **** END ADJUSTMENT ****

        // Calculate crop dimensions based on *original* box size and scaling factors
        int crop_w_lmk = static_cast<int>(box_w * BOX_SCALE_X);
        int crop_h_lmk = static_cast<int>(box_h * BOX_SCALE_Y);

        // Calculate the *desired* source rectangle using the adjusted center
        src_rect_lmk.left   = crop_center_x - crop_w_lmk / 2;
        src_rect_lmk.top    = crop_center_y - crop_h_lmk / 2; // Uses adjusted center
        src_rect_lmk.right  = src_rect_lmk.left + crop_w_lmk;
        src_rect_lmk.bottom = src_rect_lmk.top + crop_h_lmk;

        // Store the *original* desired crop ROI for post-processing *before* clamping
        box_rect_t face_crop_roi_for_post = {src_rect_lmk.left, src_rect_lmk.top, src_rect_lmk.right, src_rect_lmk.bottom};
        ...
        // Calculate effective width/height AFTER clamping
        int clamped_crop_w_lmk = clamped_src_rect_lmk.right - clamped_src_rect_lmk.left;
        int clamped_crop_h_lmk = clamped_src_rect_lmk.bottom - clamped_src_rect_lmk.top;

        // Skip if clamping results in zero or invalid size
        if (clamped_crop_w_lmk <= 0 || clamped_crop_h_lmk <= 0) {
             printf("WARN: Landmark crop rect invalid after clamping for face %d [%d,%d,%d,%d]\n", i,
                    clamped_src_rect_lmk.left, clamped_src_rect_lmk.top, clamped_src_rect_lmk.right, clamped_src_rect_lmk.bottom);
        ...
        // Perform RGA/CPU crop using the CLAMPED source rect and calculated dest rect
        ret = convert_image(src_img, &landmark_input_buf_wrapper, &clamped_src_rect_lmk, &dst_rect_lmk, bg_color);
        if (ret < 0) {
        ...
        // Post-process landmarks using the ORIGINAL, UNCLAMPED crop ROI as reference
        ret = post_process_face_landmarks(app_ctx->output_mems_landmark[0], app_ctx, face_crop_roi_for_post, &lmk_letter_box, current_face_result->face_landmarks);
        if (ret == 0) {
        ...
        // --- Stage 3: Iris Landmarks (Keep the clamping logic as corrected previously) ---
        if (current_face_result->face_landmarks_valid) {
            const int LEFT_EYE_ROI_IDX1 = 33; const int LEFT_EYE_ROI_IDX2 = 133;
            const int RIGHT_EYE_ROI_IDX1 = 362; const int RIGHT_EYE_ROI_IDX2 = 263;
            ...
            if (LEFT_EYE_ROI_IDX1 < NUM_FACE_LANDMARKS && LEFT_EYE_ROI_IDX2 < NUM_FACE_LANDMARKS) {
                ...
                if (clamped_iris_size_left_w > 0 && clamped_iris_size_left_h > 0) {
                    ...
                    if (ret == 0) {
                        ret = convert_image(src_img, &iris_input_buf_wrapper, &clamped_src_rect_iris_left, &dst_rect_iris_left, bg_color);
                        if (ret == 0) {
                            ret = rknn_set_io_mem(app_ctx->rknn_ctx_iris, app_ctx->input_mems_iris[0], &app_ctx->input_attrs_iris[0]);
                            if (ret < 0){ printf("IrisL set io err %d\n", ret); continue;}

                            // ***** FIX HERE: Preprocessor directives on separate lines *****
                            #ifdef ZERO_COPY
                            ret = rknn_mem_sync(app_ctx->rknn_ctx_iris, app_ctx->input_mems_iris[0], RKNN_MEMORY_SYNC_TO_DEVICE);
                            if (ret < 0){ printf("IrisL sync in err %d\n", ret); continue;}
                            #endif
                            // ***** END FIX *****
                            ...
                            if (ret == 0) {
                                ...
                                if(ret == 0) { // Check sync result before post-processing
                                    ret = post_process_iris_landmarks(app_ctx->output_mems_iris, app_ctx, left_eye_crop_roi_for_post, &iris_letter_box_left, current_face_result->eye_landmarks_left, current_face_result->iris_landmarks_left);
                                    if (ret == 0) { current_face_result->eye_landmarks_left_valid = true; current_face_result->iris_landmarks_left_valid = true; }
                                    else { printf("WARN: pp iris left failed %d\n", ret);}
                                } else { printf("WARN: sync iris left out failed %d\n", ret); }
                            } else { printf("WARN: run iris left failed %d\n", ret); }
                        } else { printf("WARN: convert iris left failed %d\n", ret); }
                    } else { printf("WARN: calc letterbox iris left failed %d\n", ret); }
            ...
            if (RIGHT_EYE_ROI_IDX1 < NUM_FACE_LANDMARKS && RIGHT_EYE_ROI_IDX2 < NUM_FACE_LANDMARKS) {
                ...
                if (clamped_iris_size_right_w > 0 && clamped_iris_size_right_h > 0) {
                    ...
                    if (ret == 0) {
                        ...
                        if (ret == 0) {
                            ret = rknn_set_io_mem(app_ctx->rknn_ctx_iris, app_ctx->input_mems_iris[0], &app_ctx->input_attrs_iris[0]);
                            if (ret < 0) { printf("IrisR set io err %d\n", ret); continue; }

                            // ***** FIX HERE: Preprocessor directives on separate lines *****
                            #ifdef ZERO_COPY
                            ret = rknn_mem_sync(app_ctx->rknn_ctx_iris, app_ctx->input_mems_iris[0], RKNN_MEMORY_SYNC_TO_DEVICE);
                            if (ret < 0) { printf("IrisR sync in err %d\n", ret); continue; }
                            #endif
                            // ***** END FIX *****
