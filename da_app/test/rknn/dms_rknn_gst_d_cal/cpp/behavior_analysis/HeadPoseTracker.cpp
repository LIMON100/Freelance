...
HeadPoseTracker::HeadPoseTracker() {
    // Initialize new state variables if needed (they default to false)
    currently_turned_beyond_15 = false;
    logged_30_this_turn = false;
    logged_45_this_turn = false;
}
...
int HeadPoseTracker::calculateHeadMovementKSS(int timeWindow /* = 300 */) { // Removed minDuration
    // Define thresholds CONSISTENTLY with event generation logic in run()
    const float threshold_15 = 15.0f;
    const float threshold_30 = 30.0f; // Matches KSS rule >=30 count threshold
    const float threshold_45 = 45.0f; // Matches KSS rule >=45 count threshold
    ...
    // Clean up old events from the main history deque
    while (!head_turn_events.empty() && (current_time - head_turn_events.front().timestamp > timeWindow + 10)) { // Add buffer
        head_turn_events.pop_front();
    }

    // Reset counts before recalculating
    count_15_last_calc = 0;
    count_30_last_calc = 0;
    count_45_last_calc = 0;

    if (recent_events.empty()) {
        head_movement_kss = 1; // Default KSS if no recent turns
        return head_movement_kss;
    }

    // Count based on the threshold stored in the event
    for (const auto& event : recent_events) {
        // Use the threshold stored in the event for counting
        if (event.angle_threshold_crossed >= threshold_45) {
            count_45_last_calc++;
        }
        if (event.angle_threshold_crossed >= threshold_30) { // Event logged when >=20 crossed
            count_30_last_calc++;
        }
        if (event.angle_threshold_crossed >= threshold_15) { // Event logged when >=15 crossed
            count_15_last_calc++;
        }
    }

    // Determine KSS score based on the criteria (using the counts just calculated)
    if (count_45_last_calc >= 5) {
        head_movement_kss = 7;
    } else if (count_30_last_calc >= 3) {
        head_movement_kss = 4;
    } else if (count_15_last_calc >= 3) {
        head_movement_kss = 1;
    } else {
        head_movement_kss = 1; // Default KSS
    }
...
HeadPoseTracker::HeadPoseResults HeadPoseTracker::run(const std::vector<cv::Point>& faceLandmarks) {
    ...
    results.reference_set = calibration_state.reference_set;
    if (faceLandmarks.empty()) {
        // ... (handle empty landmarks as before) ...
        std::vector<std::vector<std::string>> rows = { {"Yaw", "N/A"}, {"Pitch", "N/A"}, {"Roll", "N/A"}, {"Head KSS", "N/A"}, {"Direction", "N/A"} }; results.rows = rows; return results;
    }
    ...
    // Smoothing angles (remains the same)
    static float smoothed_yaw = 0.0f, smoothed_pitch = 0.0f, smoothed_roll = 0.0f;
    const float alpha = 0.2f; // Smoothing factor
    smoothed_yaw = (1.0f - alpha) * smoothed_yaw + alpha * yaw;
    smoothed_pitch = (1.0f - alpha) * smoothed_pitch + alpha * pitch;
    smoothed_roll = (1.0f - alpha) * smoothed_roll + alpha * roll;
    yaw = smoothed_yaw; pitch = smoothed_pitch; roll = smoothed_roll;

    // Record current pose data (remains the same)
    HeadPoseData headPoseData;
    headPoseData.yaw = yaw; headPoseData.pitch = pitch; headPoseData.roll = roll;
    headPoseData.timestamp = time(0);
    head_movement_history.push_back(headPoseData);
    if (head_movement_history.size() > 100) { head_movement_history.pop_front(); } // Limit history size

    // --- MODIFIED Event Detection Logic ---
    float max_angle = std::max(std::abs(yaw), std::abs(pitch));
    time_t current_time = time(0);
    static time_t last_event_log_time = 0;
    const float min_time_between_any_log = 0.5f; // Prevent rapid logging

    // Define thresholds used for KSS scoring
    const float threshold_15 = 15.0f;
    const float threshold_30 = 30.0f; // Threshold for the ">=30" count
    const float threshold_45 = 45.0f; // Threshold for the ">=45" count

    if (max_angle >= threshold_15) {
        // Check if we just crossed the 15 degree threshold (start of turn)
        if (!currently_turned_beyond_15) {
            currently_turned_beyond_15 = true;
            // Reset flags for higher thresholds for this new turn
            logged_30_this_turn = false;
            logged_45_this_turn = false;

            // Log the initial >=15 event if enough time has passed
            if ((current_time - last_event_log_time) >= min_time_between_any_log) {
                head_turn_events.push_back({current_time, threshold_15}); // Log the threshold value
                last_event_log_time = current_time;
            }
        }

        // Check for crossing higher thresholds *during* the current turn
        if (!logged_30_this_turn && max_angle >= threshold_30) {
            logged_30_this_turn = true;
            if ((current_time - last_event_log_time) >= min_time_between_any_log) {
                head_turn_events.push_back({current_time, threshold_30}); // Log the threshold value
                last_event_log_time = current_time;
            }
        }
        if (!logged_45_this_turn && max_angle >= threshold_45) {
            logged_45_this_turn = true;
             if ((current_time - last_event_log_time) >= min_time_between_any_log) {
                head_turn_events.push_back({current_time, threshold_45}); // Log the threshold value
                last_event_log_time = current_time;
             }
        }
    } else {
        // Head returned below 15 degrees, reset the state for the next turn
        currently_turned_beyond_15 = false;
        // Flags logged_30/45_this_turn will be reset when currently_turned_beyond_15 becomes true again
    }
    // --- End MODIFIED Event Detection ---

    // Calculate head direction (remains the same)
    HeadDirection direction = calculateHeadDirection(yaw, pitch);
    std::string direction_str;
    switch (direction) { /* ... remains the same ... */ case HeadDirection::NEUTRAL: direction_str = "Neutral"; break; case HeadDirection::LEFT: direction_str = "Left"; break; case HeadDirection::RIGHT: direction_str = "Right"; break; case HeadDirection::UP: direction_str = "Up"; break; case HeadDirection::DOWN: direction_str = "Down"; break; case HeadDirection::LEFT_UP: direction_str = "Left-Up"; break; case HeadDirection::LEFT_DOWN: direction_str = "Left-Down"; break; case HeadDirection::RIGHT_UP: direction_str = "Right-Up"; break; case HeadDirection::RIGHT_DOWN: direction_str = "Right-Down"; break; }

    // Calculate KSS Score *after* potentially adding new events
    int head_movement_kss_score = calculateHeadMovementKSS();

    // Format output strings (remains the same)
    std::stringstream stream; stream << std::fixed << std::setprecision(1);
    stream.str(std::string()); stream << yaw; std::string yaw_str = stream.str();
    stream.str(std::string()); stream << pitch; std::string pitch_str = stream.str();
    stream.str(std::string()); stream << roll; std::string roll_str = stream.str();

    std::vector<std::vector<std::string>> result_rows = {
        {"Yaw", yaw_str + " deg"}, {"Pitch", pitch_str + " deg"}, {"Roll", roll_str + " deg"},
        {"Head KSS", std::to_string(head_movement_kss_score)}, {"Direction", direction_str}
    };
    results.rows = result_rows;
