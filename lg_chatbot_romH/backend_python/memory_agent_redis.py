...
class RedisStore:
    """
    A simple wrapper around the Redis client to implement the `search` method
    """
    ...
    def put(self, namespace: tuple, key: str, value: dict):
        ...
        self.client.hset(self._format_key(namespace), key, str(value))

    def get(self, namespace: tuple, key: str):
       """Get a single item from the store"""
       value = self.client.hget(self._format_key(namespace), key)
       if value:
           return  _MemoryItem(key=key, value=eval(value.decode('utf-8')))
       return None
    
    def search(self, namespace: tuple) -> List["_MemoryItem"]:
        """Search for items by namespace."""
        items = self.client.hgetall(self._format_key(namespace))
        return [ _MemoryItem(key=key.decode('utf-8'), value=eval(value.decode('utf-8'))) for key, value in items.items()]

    def _format_key(self, namespace: tuple) -> str:
...
def chat_endpoint():
    ...
    final_state = graph.invoke(
        {"messages": messages},
        config={"configurable": {"user_id": user_id, "store":redis_store}}
    )
