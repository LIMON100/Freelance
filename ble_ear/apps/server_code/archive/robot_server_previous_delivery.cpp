...
#define DISPLAY_DEVICE "/dev/console" // Adjust this based on your display setup (e.g., /dev/fb0 for framebuffer)
const int DISPLAY_DELAY_MS = 2000; // Delay in milliseconds to show each command
...
//  This function updates the display with the latest command.
// =========================================================================
void updateDisplay(const std::string& command) {
    // Simple console output for display (adjust based on your display setup)
    std::string displayMessage = "Command Received: " + command + "\n";
    write(STDOUT_FILENO, displayMessage.c_str(), displayMessage.length());
    fsync(STDOUT_FILENO); // Ensure the write is flushed to the display

    // Optional: Add a delay to keep the command visible
    usleep(DISPLAY_DELAY_MS * 1000); // Convert milliseconds to microseconds
}
...
static gboolean check_for_commands(gpointer user_data) {
    ...
    if (command_found) {
        ...
        // Update the display with the received command
        updateDisplay(command);

        // Process the command based on the command string
