...
# Global variable to track start time of head deviation
head_deviation_start_time = None #all type

def calculate_kss_score(eye_closure_data, head_pose_data, yawn_data, head_pose_angles, head_turn_history, yawn_detector, face_occluded, landmark_confidence, object_detections):  # add obj detect data
    kss_score = 1  # Start with a base KSS score of 1 (fully alert)

    # --- Object Detection ---
    cigar_detected = any(d['class_id'] == 0 for d in object_detections)
    mobile_detected = any(d['class_id'] == 1 for d in object_detections)

    if mobile_detected:  # mobile addition
        kss_score += 2
    if cigar_detected:  # cigar additon
        kss_score += 3
    ...
    # --- Head Movement ---
    try:
        if head_pose_data[3][1] and head_pose_data[4][1]: #head
            head_horizontal = head_pose_data[3][1]
            head_vertical = head_pose_data[4][1]
        else:
            head_horizontal = ""
            head_vertical = ""
    except:
    ...
    if face_occluded:
     ...
     # --- Sunglasses / No Eye Visibility and Sustained Head Pose---
    GLOBAL_HEAD_DEVIATION_TIME = 4  # seconds

    global head_deviation_start_time

    head_deviated = head_horizontal != "Facing Forward" or head_vertical != "Facing Forward"  #any head movement with low confident
    if landmark_confidence < 0.3:  # Very low confidence in landmark detection (e.g., sunglasses)
        if head_deviated:  # If any head is deviated, then start check time count
            if head_deviation_start_time is None:
                head_deviation_start_time = time.time() #start counter

        if head_deviation_start_time is not None: #started now check
            head_deviation_duration = time.time() - head_deviation_start_time #caclute the duration. 
            if not head_deviated: #here i am making a chage head side
                head_deviation_start_time = None #if you not head down the change to reset count again.
            if head_deviation_duration >= GLOBAL_HEAD_DEVIATION_TIME:  # check the time

                print("Head deviated (any dir) and Eye occluded for Long Time")
                kss_score = 8 
                head_deviation_start_time = None #set this to none. meaning we are not increasing the kss.
...
def process_frame(frame, face_mesh, blink_detector, head_pose_tracker, yawn_detector, calibrate_done, calibration_end_time, head_turn_history):
    ...
    if mobile_detected and cigar_detected:
        overlay_text = "Mobile & Cigar Detected"
    elif mobile_detected:
        overlay_text = "Mobile Detected"
    elif cigar_detected:
        overlay_text = "Cigar Detected"

    cv2.putText(frame, overlay_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

    rows = eye_closure_ + head_pose_ + yawn_status_
    kss_score = calculate_kss_score(eye_closure_, head_pose_, yawn_status_, head_pose_angles, head_turn_history, yawn_detector, face_occluded, landmark_confidence, object_detections)  # obj

    # Determine alert message based on KSS score
    alert_message = ""
    if kss_score >= 4 and kss_score <= 6:
        alert_message = "Moderate Risk"
    elif kss_score >= 7 and kss_score <= 9:
        alert_message = "High Risk"
    elif kss_score >= 10:
        alert_message = "Extreme Risk"

    # Display alert message at the top middle of the frame
    text_size = cv2.getTextSize(alert_message, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2)[0]
    text_x = (w - text_size[0]) // 2  # Calculate horizontal center position
    cv2.putText(frame, alert_message, (text_x, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2) #Display alert text
