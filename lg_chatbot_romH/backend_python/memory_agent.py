...
# class PostgresStore:
#     """
#     A wrapper around the PostgreSQL database to implement the `search`, `put` and `get` methods.
#     """
#     def __init__(self):
...
#        """Store a value in the PostgreSQL table."""
#        try:
#          with self.conn.cursor() as cur:
#             cur.execute(
#                 "INSERT INTO user_preferences (user_id, platform_preference) VALUES (%s, %s) ON CONFLICT (user_id) DO UPDATE SET platform_preference = %s",
#                 (namespace[1], str(value), str(value)),
#             )
#             self.conn.commit()
#        except Exception as e:
#          print(f"Error when putting value to the database, {e}")
...
#            with self.conn.cursor() as cur:
#             cur.execute(
#               "SELECT platform_preference FROM user_preferences WHERE user_id = %s",
#               (namespace[1],),
#             )
#             result = cur.fetchone()
#             if result:
#                 return _MemoryItem(key=key, value=eval(result[0]))
#             return None
#         except Exception as e:
#             print(f"Error when fetching value to the database, {e}")
...
#             )
#             rows = cur.fetchall()
#             return [ _MemoryItem(key=namespace[1], value=eval(row[0])) for row in rows]
#         except Exception as e:
#           print(f"Error when fetching all values from the database, {e}")
