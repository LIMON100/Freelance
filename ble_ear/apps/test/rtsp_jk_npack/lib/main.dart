...
// --- MAIN APP ENTRY POINT (No changes) ---
void main() async {
  WidgetsFlutterBinding.ensureInitialized();
  SystemChrome.setEnabledSystemUIMode(SystemUiMode.manual, overlays: []);
  SystemChrome.setPreferredOrientations([
...
class _HomePageState extends State<HomePage> {
  ...
  // FIXED: This method was missing in the previous version
  Future<void> _handleGamepadEvent(MethodCall call) async {
    if (!mounted) return;
    ...
    if (call.method == "onMotionEvent") {
      ...
      switch (button) {
        ...
        case 'KEYCODE_BUTTON_X': _onPermissionPressed(); break;
      }
    }
  }
  ...
  void _startCommandTimer() {
    _commandTimer = Timer.periodic(const Duration(milliseconds: 100), (timer) {
      ...
      if (_gamepadConnected) {
        ...
        double triggerValue = _gamepadAxisValues['AXIS_RTRIGGER'] ?? _gamepadAxisValues['AXIS_LTRIGGER'] ?? 0.0;
        _currentCommand.gun_trigger = triggerValue > 0.5;
        if ((_gamepadAxisValues['KEYCODE_BUTTON_L1'] ?? 0.0) > 0.5) _currentCommand.zoom_command = 1;
        if ((_gamepadAxisValues['KEYCODE_BUTTON_L2'] ?? 0.0) > 0.5 || (_gamepadAxisValues['AXIS_BRAKE'] ?? 0.0) > 0.5) _currentCommand.zoom_command = -1;
      } else {
        ...
        _currentCommand.gun_trigger = false;
      }
      _sendCommandPacket(_currentCommand);
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 5 * heightScale),
              Text(label, textAlign: TextAlign.center, style: TextStyle(fontFamily: 'NotoSans', fontWeight: FontWeight.bold, fontSize: 24 * heightScale, color: Colors.white)),
            ],
          ),
        ),
      ),
      ...
      builder: (context, constraints) {
        final List<Widget> leftCluster = [
          _buildBottomBarButton(
            ...
            null, // <-- FIX: Pass null for no icon
          ...
          _buildWideBottomBarButton(
            ...
            _isModeActive ? ICON_PATH_STOP : ICON_PATH_START, // <-- Corrected icons
            [const Color(0xff25a625), const Color(0xff127812)],
            _onStartStopPressed,
          ),
          SizedBox(width: 12 * widthScale),
  ...
  Widget _buildBottomBarButton(String label, String? iconPath, List<Color> gradientColors, VoidCallback onPressed) { // <-- Change to String?
    final screenHeight = MediaQuery.of(context).size.height;
    final heightScale = screenHeight / 1080.0;
    ...
    return GestureDetector(
      ...
      child: Container(
        height: 80 * heightScale,
        padding: EdgeInsets.symmetric(horizontal: 30), // Use consistent padding
        decoration: BoxDecoration(
          gradient: LinearGradient(colors: gradientColors, begin: Alignment.topCenter, end: Alignment.bottomCenter),
          borderRadius: BorderRadius.circular(25 * heightScale),
        ),
  ...
  Widget _buildTouchDetector() {
    return GestureDetector(
      ...
      onTapUp: (details) {
        if (_isModeActive && _selectedModeIndex == 2) { // 2 = Manual Attack
          ...
          if (renderBox != null) {
            ...
            if (x >= 0.0 && x <= 1.0 && y >= 0.0 && y <= 1.0) {
              setState(() {
                _currentCommand.touch_x = x;
                _currentCommand.touch_y = y;
              });
            }
          }
        }
