...
class _MembershipPageState extends State<MembershipActivePlanPage> {
  ...
  Widget membership(){
    ...
    if (auth.value.membership!.duration.contains("week")) {
      return ButtonPrimaryWidget(
        'Upgrade Membership',
        marginHorizontal: 50,
        marginVertical: 50,
        onTap: () {
          Get.to(MembershipPage(data: auth.value, isUpgrade: true));
        },);
    } else if (auth.value.membership!.duration.contains("6 months")) {
      return ButtonPrimaryWidget('Downgrade Membership',
        marginHorizontal: 50,
        marginVertical: 50,onTap: () {
          Get.to(MembershipPage(data: auth.value, isDowngrade: true));
        },);
    }else{
      return
        Column(
          children: [
            ButtonPrimaryWidget(
              'Upgrade Membership',
              marginHorizontal: 20,
              marginVertical: 5, // Reduced to 5 to make them closer
              onTap: () {
                Get.to(MembershipPage(data: auth.value, isUpgrade: true));
              },
            ),
