...
#define TOUCH_PORT 65433 // NEW PORT FOR TOUCH COORDINATES

#pragma pack(push, 1)
// Main 5-byte command struct
struct UserCommand {
    uint8_t command_id;
...
// NEW: 8-byte touch coordinate struct
struct TouchCoord {
    float x;
    float y;
};
...
// NEW: Separate queue for touch commands
std::queue<TouchCoord> touch_queue;
std::mutex touch_queue_mutex;

// =========================================================================
//  COMMAND SERVER THREAD (for UserCommand)
// =========================================================================
void commandServerThread() {
...
// =========================================================================
//  NEW: TOUCH COORDINATE SERVER THREAD
// =========================================================================
void touchServerThread() {
    ...
    if ((server_fd = socket(AF_INET, SOCK_STREAM, 0)) == 0) {
        perror("[Touch Thread] socket failed"); return;
    }
    if (setsockopt(server_fd, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT, &opt, sizeof(opt))) {
        perror("[Touch Thread] setsockopt"); return;
    }
    address.sin_family = AF_INET;
    address.sin_addr.s_addr = INADDR_ANY;
    address.sin_port = htons(TOUCH_PORT);
    if (bind(server_fd, (struct sockaddr *)&address, sizeof(address)) < 0) {
        perror("[Touch Thread] bind failed"); return;
    }
    if (listen(server_fd, 3) < 0) {
        perror("[Touch Thread] listen"); return;
    }

    std::cout << "[Touch Thread] Listening for touch commands on port " << TOUCH_PORT << std::endl;

    while (true) {
        if ((new_socket = accept(server_fd, (struct sockaddr *)&address, &addrlen)) < 0) {
            perror("[Touch Thread] accept"); continue;
        }
        
        TouchCoord received_touch;
        int valread = read(new_socket, &received_touch, sizeof(TouchCoord));
        
        if (valread == sizeof(TouchCoord)) {
            std::lock_guard<std::mutex> lock(touch_queue_mutex);
            touch_queue.push(received_touch);
        }
        close(new_socket);
    }
}
...
// =========================================================================
//  GSTREAMER CALLBACK (UPDATED to check both queues)
// =========================================================================
static gboolean check_for_commands(gpointer user_data) {
    // Check for main user commands
    UserCommand command;
    bool command_found = false;
    {
    ...
    if (command_found) {
        std::cout << "--- Processing Command Packet ---" << std::endl;
        // ... (rest of processing logic is the same)
    }

    // NEW: Check for touch commands
    TouchCoord touch;
    bool touch_found = false;
    {
        std::lock_guard<std::mutex> lock(touch_queue_mutex);
        if (!touch_queue.empty()) {
            touch = touch_queue.front();
            touch_queue.pop();
            touch_found = true;
        }
    }
    if (touch_found) {
        std::cout << "--- Processing Touch Packet ---" << std::endl;
        std::cout << "  Normalized X: " << touch.x << std::endl;
        std::cout << "  Normalized Y: " << touch.y << std::endl;
        std::cout << "-------------------------------" << std::endl;
        // TODO: Add logic to aim robot using these coordinates
    }
...
// =========================================================================
//  MAIN APPLICATION (UPDATED to launch both threads)
// =========================================================================
int main(int argc, char *argv[]) {
    gst_init(&argc, &argv);

    // Launch BOTH server threads
    std::thread cmd_thread(commandServerThread);
