...
void init_video_writer(const std::string &output_path, cv::VideoWriter &video, double fps, int org_width, int org_height) {
    // OLD, INCOMPATIBLE WAY:
    // int fourcc = cv::VideoWriter::fourcc('m', 'p', '4', 'v');

    // NEW, WEB-COMPATIBLE WAY: Use the FourCC for H.264/AVC
    int fourcc = cv::VideoWriter::fourcc('a', 'v', 'c', '1');
    
    video.open(output_path, fourcc, fps, cv::Size(org_width, org_height));
    
    // CRITICAL: Add this check to see if the writer opened successfully.
    // If your system is missing the necessary codec library (like openh264), this will fail.
    if (!video.isOpened()) {
        std::cerr << "************************************************************" << std::endl;
