...
//     std::string input_video_path = "./100m.mp4";
...
struct PipelineConfig {
    // --- SOURCE CONFIGURATION ---
    // FIX: Paths for both EO and IR streams
    std::string eo_video_path = "./75eo.mp4"; // Example path for EO video
    std::string ir_video_path = "./75ir.mp4"; // Example path for IR video
    
    // FIX: Path to the calibration file that maps IR coordinates to EO coordinates
    ...
    bool use_live_stream = false;
    bool enable_profiling_log = true;
    bool enable_global_stabilization = true; 
    bool enable_visualization = true;
    bool save_output_video = false; 
