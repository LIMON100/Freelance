...
def process_image(image_path, face_mesh, blink_detector, head_pose_tracker, yawn_detector):
    try:
        ...
        if results.multi_face_landmarks:
            ...
            # eye_state = "open" if not any(item[1] == 'Yes' for item in eye_closure if item[0] in ('Eye_left', 'Eye_right')) else "closed"
        ...
        for i, text in enumerate(overlay_text):
            cv2.putText(frame, text, (10, y_offset + i * 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, green, 2, cv2.LINE_AA)
...
def main():
    ...
    image_folder = "/home/limonubuntu/Work/Limon/other_task/driver_anomaly/test/test_dataset/test_real_images/extracted_frames/"
    output_folder = "output_images33"
