...
void TrackingPipeline::fusion_worker() {
    while (!m_stop_flag.load()) {
        SingleStreamFrameData eo_item, ir_item;
        // Use a timed pop to prevent one stream from blocking the other indefinitely
        // This is an advanced change, for now we'll stick to the original pop
        if (!m_results_queue_eo->pop(eo_item) || !m_results_queue_ir->pop(ir_item)) {
            break; // One of the streams has ended
        }

        FusedFrameData fused_item;
        fused_item.frame_id = eo_item.frame_id;
        // The display frame is always the original EO frame
        fused_item.display_frame = eo_item.org_frame.clone(); // Use a clone to be safe

        // Copy timestamps for profiling
        fused_item.t_eo_capture_start = eo_item.t_capture_start;
        fused_item.t_eo_preprocess_end = eo_item.t_preprocess_end;
        fused_item.t_eo_inference_end = eo_item.t_inference_end;
        fused_item.t_ir_capture_start = ir_item.t_capture_start;
        ...
        // 1. Get EO detections in the full original frame coordinate space
        int eo_proc_width = m_config.enable_center_crop ? m_config.crop_size : eo_item.org_frame.cols;
        int eo_proc_height = m_config.enable_center_crop ? m_config.crop_size : eo_item.org_frame.rows;
        auto eo_bboxes = parse_nms_data(eo_item.output_data_and_infos[0].first, m_config.class_count);
        std::vector<byte_track::Object> eo_objects = detections_to_bytetrack_objects(eo_bboxes, eo_proc_width, eo_proc_height);
        ...
        for (const auto& ir_obj_raw : ir_objects_raw) {
            byte_track::Object ir_obj = ir_obj_raw; // Make a mutable copy
            ...
            // Check if the boresight matrix is valid before transforming
            if (!m_boresight_matrix.empty()) {
                cv::perspectiveTransform(ir_corners, eo_corners, m_boresight_matrix);
        ...
        // --- NEW FUSION LOGIC: Prioritize confirmed detections ---
        
        const float fusion_iou_thresh = 0.3f; // A stricter threshold helps reduce false matches
        std::vector<bool> eo_matched(eo_objects.size(), false);
        std::vector<bool> ir_matched(ir_objects_projected.size(), false);

        // **Step 3A: Find High-Confidence Fused Detections**
        // These are detections seen by BOTH cameras. They are the most reliable.
        for (size_t i = 0; i < eo_objects.size(); ++i) {
            for (size_t j = 0; j < ir_objects_projected.size(); ++j) {
                // Skip if this IR detection has already been matched
                if (ir_matched[j]) {
                    continue;
                }
                
                if (eo_objects[i].rect.calcIoU(ir_objects_projected[j].rect) > fusion_iou_thresh) {
                    // This is a match! We create a single, high-confidence fused object.
                    // Let's average the boxes and take the highest score for stability.
                    ...
                    fused_item.fused_detections.emplace_back(
                        ...
                        eo_objects[i].label, // Assume label is the same
                        fused_score
                    );
                    
                    eo_matched[i] = true;
                    ir_matched[j] = true;
                    break; // This EO object is matched, move to the next one.
                }
            }
        }

        // **Step 3B (Optional but Recommended): Add Unmatched High-Confidence Detections**
        // Sometimes, an object is clearly visible in one camera but not the other.
        // We only add these if their confidence score is high, to avoid noise.
        const float single_source_confidence_thresh = 0.55f; // Configurable threshold

        // Add any high-confidence, unmatched EO detections.
        for (size_t i = 0; i < eo_objects.size(); ++i) {
        ...
        // Add any high-confidence, unmatched IR detections.
        for (size_t j = 0; j < ir_objects_projected.size(); ++j) {
