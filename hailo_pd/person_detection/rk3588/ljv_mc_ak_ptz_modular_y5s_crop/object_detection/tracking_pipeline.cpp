...
struct GstCallbackData {
    std::shared_ptr<BoundedTSQueue<cv::Mat>> queue; // The queue to push frames into
...
//         /// debugger
//         if (m_config.enable_profiling_log && !bboxes.empty()) {
//             const auto& first_bbox = bboxes[0].bbox;
//             std::cout << "[DEBUG Postproc] Raw YOLO detection (normalized): "
//                     << "x_min=" << first_bbox.x_min << ", y_min=" << first_bbox.y_min
//                     << ", x_max=" << first_bbox.x_max << ", y_max=" << first_bbox.y_max
//                     << std::endl;
//         }
...
//         // --- THIS IS THE FIX: Call the correct bridge function with all necessary size info ---
//         std::vector<byte_track::Object> objects = detections_to_bytetrack_objects(
//             bboxes, 
//             m_config.model_input_width,   // The size the model sees (e.g., 640)
//             m_config.model_input_height,  // e.g., 640
//             proc_width,                   // The size of the pre-resized image (e.g., 500)
//             proc_height,                  // e.g., 500
//             item.crop_offset              // The offset of the crop in the full frame
//         );
//         // --- END OF FIX ---

//         /// debugger
//         if (m_config.enable_profiling_log && !objects.empty()) {
//             const auto& first_obj = objects[0].rect;
//             std::cout << "[DEBUG Postproc] Coords sent to tracker (full frame): "
//                     << "x=" << first_obj.x() << ", y=" << first_obj.y()
//                     << ", w=" << first_obj.width() << ", h=" << first_obj.height()
//                     << std::endl;
//         }

//         std::vector<std::shared_ptr<byte_track::STrack>> tracked_objects = m_tracker->update(objects, item.affine_matrix);

//         // --- TIMESTAMP 4: Postprocess End ---
//         item.t_postprocess_end = std::chrono::high_resolution_clock::now();
...
//         if (m_config.enable_visualization) {
//             // --- DEBUGGER: Draw the raw, transformed detections in BLUE ---
//             for (const auto& obj : objects) {
//                 const auto& rect = obj.rect;
//                 cv::Rect2f raw_bbox(rect.x(), rect.y(), rect.width(), rect.height());
//                 cv::rectangle(frame_to_draw, raw_bbox, cv::Scalar(255, 0, 0), 1); // Blue, thin rectangle
//             }
//             // --- END DEBUGGER ---
...
//             // Drawing all (GREEN) tracked objects
//             for (const auto& track : tracked_objects) {
//                 const byte_track::Rect<float>& rect = track->getRect();

//                 // --- DEBUGGER: Print the final coordinates from the tracker before drawing ---
//                 if (m_config.enable_profiling_log) {
//                     std::cout << "[DEBUG Postproc] Final Tracked Coords (ID " << track->getTrackId() << "): "
//                             << "x=" << rect.x() << ", y=" << rect.y()
...
//     const std::vector<NamedBbox>& bboxes, 
//     int model_input_width, int model_input_height, // Size of the image fed to the model (e.g., 640x640)
//     int processed_frame_width, int processed_frame_height, // Size of the image BEFORE model resize (e.g., 500x500 or 1920x1080)
//     const cv::Point& crop_offset)                  // Top-left corner of the crop in the full raw frame
// {
//     std::vector<byte_track::Object> objects;
//     objects.reserve(bboxes.size());
...
//     for (const auto& bbox : bboxes) {
//         // Step 1: De-normalize the coordinates from the model's 0.0-1.0 range
//         // This gives us the bounding box in pixels, relative to the model's input size (e.g., 640x640).
//         float x1_model = bbox.bbox.x_min * model_input_width;
//         float y1_model = bbox.bbox.y_min * model_input_height;
//         float x2_model = bbox.bbox.x_max * model_input_width;
//         float y2_model = bbox.bbox.y_max * model_input_height;

//         // Step 2: Scale these pixel coordinates from the model's space back to the processed frame's space.
//         // This correctly handles any aspect ratio distortion from resizing.
//         float x1_proc = x1_model * scale_x;
//         float y1_proc = y1_model * scale_y;
//         float x2_proc = x2_model * scale_x;
//         float y2_proc = y2_model * scale_y;
