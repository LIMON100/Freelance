...
// #define BOX_SCALE_X 1.5
// #define BOX_SCALE_Y 1.7

// struct AnchorOptions {
//     // Input size used by the model generating these anchors
//     int input_size_width = 128; // Match app_ctx->model1_width
//     int input_size_height = 128; // Match app_ctx->model1_height

//     // Strides and feature map sizes
//     std::vector<int> strides = {8, 16}; // Example strides
//     // Calculate feature map sizes based on strides
//     // int feature_map_width_0 = ceil((float)input_size_width / strides[0]); -> 16
//     // int feature_map_height_0 = ceil((float)input_size_height / strides[0]); -> 16
//     // int feature_map_width_1 = ceil((float)input_size_width / strides[1]); -> 8
//     // int feature_map_height_1 = ceil((float)input_size_height / strides[1]); -> 8

//     int num_layers = 2; // Number of layers/strides

//     // Number of anchors per location per layer
//     // Typically 2 for stride 8, 6 for stride 16 in MediaPipe Face Detection = 896 anchors total (16*16*2 + 8*8*6 = 512 + 384 = 896)
...
//     // Fixed anchor size (often 1.0 for normalized anchors before scaling)
//     float fixed_anchor_size = 1.0f;
// };

// // Represents a generated anchor (normalized coordinates 0-1)
// struct Anchor {
//     float x_center, y_center, h, w;
// };
...
//                     float w = options.fixed_anchor_size; // Width/Height are often relative to stride or fixed
//                     float h = options.fixed_anchor_size; // In some schemes, might vary

//                     // Placeholder: Real implementations might vary h/w based on layer/anchor_id
//                     // For simplicity here, we assume fixed normalized size before regression.
//                     // The regression output will scale these.
...
//     {
//         printf("ERROR: Invalid arguments for crop_image.\n");
//         return -1;
//     }
...
//         format_supported = true;
//         channels = 1;
//         printf("WARN: Cropping grayscale image.\n");
//     }

//     if (!format_supported)
//     {
//         printf("ERROR: Unsupported image format %d for crop_image.\n", src_img->format);
//         return -1;
//     }
...
//         {
//             printf("ERROR: Failed to allocate memory for cropped image buffer.\n");
//             return -1;
//         }
//     }
...
//     size_t src_stride = src_w * channels;
//     size_t dst_stride = crop_w * channels;
...
//         unsigned char* src_row_ptr = src_data + (size_t)src_y * src_stride;
//         unsigned char* dst_row_ptr = dst_data + (size_t)y * dst_stride;
...
//             {
//                 memcpy(dst_row_ptr + x * channels, src_row_ptr + src_x * channels, channels);
//             }
//         }
//     }
...
// static int post_process_face_detection(rknn_output outputs[], rknn_app_context_t *app_ctx,
//                                       const letterbox_t *det_letter_box, int src_img_width,
//                                       int src_img_height, std::vector<DetectionCandidate>& out_faces)
// {
//     // ... (Anchor generation, output info checks remain the same) ...
//     AnchorOptions anchor_opts; // ... generate anchors ...
...
//     // ... (Dimension checks, null checks, anchor count check) ...
//     int num_anchors = box_attr->dims[1];
//     int box_regressors_per_anchor = box_attr->dims[box_attr->n_dims - 1]; // 16

//     int8_t* box_data = (int8_t*)outputs[0].buf;
//     int8_t* score_data = (int8_t*)outputs[1].buf;
//     float box_scale = box_attr->scale; int32_t box_zp = box_attr->zp;
//     float score_scale = score_attr->scale; int32_t score_zp = score_attr->zp;

//     std::vector<DetectionCandidate> candidates; candidates.reserve(num_anchors);

//     // *** Use Model Input Size as Scale Factor (MediaPipe Style) ***
//     const float scale_factor = (float)app_ctx->model1_width; // e.g., 128.0f

//     int high_score_anchor_idx = -1; float max_score_debug = -1.0f; // For Debug print

//     for (int i = 0; i < num_anchors; ++i) {
//         float logit = dequantize_int8_to_float(score_data[i], score_zp, score_scale);
//         float score = 1.0f / (1.0f + expf(-logit)); // Sigmoid

//         if(score > max_score_debug) { max_score_debug = score; high_score_anchor_idx = i; } // Debug tracking

//         if (score >= FACE_CONF_THRESHOLD) {
//             int box_offset = i * box_regressors_per_anchor;
//             // Dequantize first 4 regressors - ASSUMING order [x_offset, y_offset, w, h]
//             // These are likely offsets/scales relative to the anchor *center* and anchor *size*.
...
//             float anchor_x_center = anchor.x_center; float anchor_y_center = anchor.y_center;
//             // IMPORTANT: Anchor w/h might need adjustment. If generate_anchors uses 1.0,
//             // we might need to use the stride or relative size here instead. Let's try 1.0 first.
//             float anchor_w = anchor.w; float anchor_h = anchor.h;

//             // *** Apply MediaPipe-Style Decoding Formula ***
//             float pred_x_center = (x_offset / scale_factor * anchor_w) + anchor_x_center;
//             float pred_y_center = (y_offset / scale_factor * anchor_h) + anchor_y_center;
//             // Width/Height decoding might be direct scaling or exponential. Start with direct scaling.
//             float pred_w = (w_scale / scale_factor) * anchor_w;
//             float pred_h = (h_scale / scale_factor) * anchor_h;
//             // ALTERNATIVE Exponential (if direct scaling is wrong):
//             // float pred_w = expf(w_scale / scale_factor) * anchor_w;
//             // float pred_h = expf(h_scale / scale_factor) * anchor_h;


//             // Convert center/width/height to ymin, xmin, ymax, xmax (normalized 0-1)
//             float ymin_norm = pred_y_center - pred_h / 2.0f; float xmin_norm = pred_x_center - pred_w / 2.0f;
//             float ymax_norm = pred_y_center + pred_h / 2.0f; float xmax_norm = pred_x_center + pred_w / 2.0f;
//             // *** End Decoding Formula ***

//             // --- Coordinate transformation to original image (remains the same) ---
...
//             float model_input_w = (float)app_ctx->model1_width; float model_input_h = (float)app_ctx->model1_height;
//             float xmin_unscaled = (xmin_norm * model_input_w - det_letter_box->x_pad) / det_letter_box->scale;
//             float ymin_unscaled = (ymin_norm * model_input_h - det_letter_box->y_pad) / det_letter_box->scale;
//             float xmax_unscaled = (xmax_norm * model_input_w - det_letter_box->x_pad) / det_letter_box->scale;
//             float ymax_unscaled = (ymax_norm * model_input_h - det_letter_box->y_pad) / det_letter_box->scale;
//             int xmin = clamp((int)roundf(xmin_unscaled), 0, src_img_width - 1); int ymin = clamp((int)roundf(ymin_unscaled), 0, src_img_height - 1);
//             int xmax = clamp((int)roundf(xmax_unscaled), 0, src_img_width - 1); int ymax = clamp((int)roundf(ymax_unscaled), 0, src_img_height - 1);
//             // --- End Coordinate transformation ---

//             if (xmax > xmin && ymax > ymin) {
//                  DetectionCandidate cand; /* ... fill cand ... */
//                  cand.index = i; cand.score = score; cand.box.left = xmin; cand.box.top = ymin; cand.box.right = xmax; cand.box.bottom = ymax;
//                  candidates.push_back(cand);
//             }
//         }
//     }

//     // --- Add detailed debug print for the highest scoring anchor BEFORE NMS (UPDATED) ---
//     if (high_score_anchor_idx >= 0) {
//          printf("\n--- DEBUG: Highest Score Anchor Info (Index: %d, Score: %.4f) ---\n", high_score_anchor_idx, max_score_debug);
//          int box_offset = high_score_anchor_idx * box_regressors_per_anchor;
//          // Print DEQUANTIZED values using the NEW assumed order
//          float x_offset = dequantize_int8_to_float(box_data[box_offset + 0], box_zp, box_scale);
//          float y_offset = dequantize_int8_to_float(box_data[box_offset + 1], box_zp, box_scale);
//          float w_scale  = dequantize_int8_to_float(box_data[box_offset + 2], box_zp, box_scale);
//          float h_scale  = dequantize_int8_to_float(box_data[box_offset + 3], box_zp, box_scale);
//          const Anchor& anchor = anchors[high_score_anchor_idx];
//          printf("  Raw Regressors (dequant): x_off=%.3f, y_off=%.3f, w_scl=%.3f, h_scl=%.3f\n", x_offset, y_offset, w_scale, h_scale); // Updated labels
//          printf("  Anchor (norm): cx=%.3f, cy=%.3f, w=%.3f, h=%.3f\n", anchor.x_center, anchor.y_center, anchor.w, anchor.h);
//          // Recalculate decoded box for print using NEW formula
//          float pred_x_center = (x_offset / scale_factor * anchor.w) + anchor.x_center;
//          float pred_y_center = (y_offset / scale_factor * anchor.h) + anchor.y_center;
//          float pred_w = (w_scale / scale_factor) * anchor.w; // Using direct scaling here
//          float pred_h = (h_scale / scale_factor) * anchor.h;
//          float ymin_norm = pred_y_center - pred_h / 2.0f; float xmin_norm = pred_x_center - pred_w / 2.0f;
//          float ymax_norm = pred_y_center + pred_h / 2.0f; float xmax_norm = pred_x_center + pred_w / 2.0f;
//          printf("  Decoded Box (norm TLBR): xmin=%.4f, ymin=%.4f, xmax=%.4f, ymax=%.4f\n", xmin_norm, ymin_norm, xmax_norm, ymax_norm);
//          // Recalculate unscaled for print
//          float model_input_w = (float)app_ctx->model1_width; float model_input_h = (float)app_ctx->model1_height;
//          // Use the actual det_letter_box values passed to the function
//          float xmin_unscaled = (clampf(xmin_norm,0,1) * model_input_w - det_letter_box->x_pad) / det_letter_box->scale;
//          float ymin_unscaled = (clampf(ymin_norm,0,1) * model_input_h - det_letter_box->y_pad) / det_letter_box->scale;
//          float xmax_unscaled = (clampf(xmax_norm,0,1) * model_input_w - det_letter_box->x_pad) / det_letter_box->scale;
//          float ymax_unscaled = (clampf(ymax_norm,0,1) * model_input_h - det_letter_box->y_pad) / det_letter_box->scale;
//          printf("  Unscaled Box (before clamp): xmin=%.1f, ymin=%.1f, xmax=%.1f, ymax=%.1f\n", xmin_unscaled, ymin_unscaled, xmax_unscaled, ymax_unscaled);
//          printf("  Letterbox Params Used: scale=%.3f, x_pad=%.1f, y_pad=%.1f\n", det_letter_box->scale, det_letter_box->x_pad, det_letter_box->y_pad); // Print actual used values
//          printf("---------------------------------------------------------------\n\n");
//     }
//     // --- End Debug Print ---
...
//     out_faces = candidates;
//     return 0;
// }


// // WORKABLE
// static int post_process_face_landmarks(rknn_output landmark_output, // Only the landmark tensor output
//                                       rknn_app_context_t *app_ctx,
//                                       const box_rect_t& crop_roi, // Pass the ROI used for cropping
//                                       const letterbox_t *lmk_letter_box, // Use this!
//                                       point_t out_landmarks[NUM_LANDMARKS])
// {
//     rknn_tensor_attr* landmark_attr = &app_ctx->output_attrs_landmark[0];

//     // --- Dimension Checks (remains the same) ---
//      if (!((landmark_attr->n_dims == 4 && landmark_attr->dims[0] == 1 && landmark_attr->dims[1] == 1 && landmark_attr->dims[2] == 1 && landmark_attr->dims[3] == NUM_LANDMARKS * 3) ||
//           (landmark_attr->n_dims == 2 && landmark_attr->dims[0] == 1 && landmark_attr->dims[1] == NUM_LANDMARKS * 3)) )
//     { /* Error check */ printf("ERROR: Unexpected landmark output dims\n"); return -1; }
//     int num_coords = landmark_attr->dims[landmark_attr->n_dims - 1];
//     if (num_coords != NUM_LANDMARKS * 3) { /* Error check */ printf("ERROR: Unexpected landmark output coord count\n"); return -1; }
//     // --- End Dimension Checks ---

//     int8_t* landmark_data = (int8_t*)landmark_output.buf;
//     float landmark_scale_zp = landmark_attr->scale; // Renamed variable for clarity
//     int32_t landmark_zp = landmark_attr->zp;
//     float model_input_w = (float)app_ctx->model2_width;   // Landmark model's input W (e.g., 192)
//     float model_input_h = (float)app_ctx->model2_height;  // Landmark model's input H (e.g., 192)

//     // Get the dimensions of the *original* crop ROI
//     float crop_roi_w = (float)(crop_roi.right - crop_roi.left);
//     float crop_roi_h = (float)(crop_roi.bottom - crop_roi.top);
//     if (crop_roi_w <= 0 || crop_roi_h <= 0) { /* Error check */ printf("ERROR: Invalid crop ROI dims\n"); return -1; }


//     for (int i = 0; i < NUM_LANDMARKS; ++i)
//     {
//         int offset = i * 3;
//         // Dequantize landmark coordinate values
//         float x_lmk_quant = dequantize_int8_to_float(landmark_data[offset + 0], landmark_zp, landmark_scale_zp);
//         float y_lmk_quant = dequantize_int8_to_float(landmark_data[offset + 1], landmark_zp, landmark_scale_zp);

//         // --- START: Combined Scaling Logic ---

//         // 1. Assume x_lmk_quant, y_lmk_quant are coordinates relative to the landmark model input size (e.g., 0-191)

//         // 2. Remove the padding applied by letterbox
//         float x_scaled_crop = x_lmk_quant - lmk_letter_box->x_pad;
//         float y_scaled_crop = y_lmk_quant - lmk_letter_box->y_pad;

//         // 3. Divide by the letterbox scale factor to get coordinates relative
//         //    to the *original* unscaled cropped image dimensions.
//         float x_orig_crop_rel = x_scaled_crop / lmk_letter_box->scale;
//         float y_orig_crop_rel = y_scaled_crop / lmk_letter_box->scale;

//         // 4. Add the offset of the crop ROI's top-left corner to get final coordinates
//         //    relative to the original full image.
//         float final_x = x_orig_crop_rel + crop_roi.left;
//         float final_y = y_orig_crop_rel + crop_roi.top;

//         out_landmarks[i].x = (int)(final_x + 0.5f); // Round
//         out_landmarks[i].y = (int)(final_y + 0.5f); // Round
//     }
...
// int init_face_analyzer_models(const char *detection_model_path, const char* landmark_model_path, 
//                              rknn_app_context_t *app_ctx)
// {
...
//     model_len = read_data_from_file(detection_model_path, reinterpret_cast<char**>(&model));
    
//     if (model == NULL)
//     {
//         printf("ERROR: Failed to load face detection model!\n");
//         return -1;
//     }

//     ret = rknn_init(&app_ctx->rknn_ctx_detect, model, model_len, 0, NULL);
//     free(model);
//     model = NULL;
...
//     ret = rknn_query(app_ctx->rknn_ctx_detect, RKNN_QUERY_IN_OUT_NUM, 
//                     &app_ctx->io_num_detect, sizeof(app_ctx->io_num_detect));
                    
//     if (ret != RKNN_SUCC)
//     {
//         printf("ERROR: rknn_query(RKNN_QUERY_IN_OUT_NUM) for detection model failed! ret=%d\n", ret);
//         goto cleanup_detect;
//     }
...
//     for (int i = 0; i < app_ctx->io_num_detect.n_input; i++)
//     {
//         app_ctx->input_attrs_detect[i].index = i;
//         ret = rknn_query(app_ctx->rknn_ctx_detect, RKNN_QUERY_INPUT_ATTR, 
//                         &app_ctx->input_attrs_detect[i], sizeof(rknn_tensor_attr));
                        
//         if (ret != RKNN_SUCC)
//         {
//             printf("ERROR: rknn_query(RKNN_QUERY_INPUT_ATTR) for detect input %d failed! ret=%d\n", i, ret);
//             goto cleanup_detect;
//         }
...
//     for (int i = 0; i < app_ctx->io_num_detect.n_output; i++)
//     {
//         app_ctx->output_attrs_detect[i].index = i;
//         ret = rknn_query(app_ctx->rknn_ctx_detect, RKNN_QUERY_OUTPUT_ATTR, 
//                         &app_ctx->output_attrs_detect[i], sizeof(rknn_tensor_attr));
                        
//         if (ret != RKNN_SUCC)
//         {
//             printf("ERROR: rknn_query(RKNN_QUERY_OUTPUT_ATTR) for detect output %d failed! ret=%d\n", i, ret);
//             goto cleanup_detect;
//         }
...
//     {
//         printf("Detection model is NCHW input fmt\n");
...
//     else
//     {
//         printf("Detection model is NHWC input fmt\n");
...
//     model_len = read_data_from_file(landmark_model_path, reinterpret_cast<char**>(&model));
    
//     if (model == NULL)
//     {
//         printf("ERROR: Failed to load face landmark model!\n");
//         goto cleanup_detect;
//     }

//     ret = rknn_init(&app_ctx->rknn_ctx_landmark, model, model_len, 0, NULL);
//     free(model);
//     model = NULL;
...
//     ret = rknn_query(app_ctx->rknn_ctx_landmark, RKNN_QUERY_IN_OUT_NUM, 
//                     &app_ctx->io_num_landmark, sizeof(app_ctx->io_num_landmark));
                    
//     if (ret != RKNN_SUCC)
//     {
//         printf("ERROR: rknn_query(RKNN_QUERY_IN_OUT_NUM) for landmark model failed! ret=%d\n", ret);
//         goto cleanup_landmark;
//     }
...
//     for (int i = 0; i < app_ctx->io_num_landmark.n_input; i++)
//     {
//         app_ctx->input_attrs_landmark[i].index = i;
//         ret = rknn_query(app_ctx->rknn_ctx_landmark, RKNN_QUERY_INPUT_ATTR, 
//                         &app_ctx->input_attrs_landmark[i], sizeof(rknn_tensor_attr));
                        
//         if (ret != RKNN_SUCC)
//         {
//             printf("ERROR: rknn_query(RKNN_QUERY_INPUT_ATTR) for landmark input %d failed! ret=%d\n", i, ret);
//             goto cleanup_landmark;
//         }
...
//     for (int i = 0; i < app_ctx->io_num_landmark.n_output; i++)
//     {
//         app_ctx->output_attrs_landmark[i].index = i;
//         ret = rknn_query(app_ctx->rknn_ctx_landmark, RKNN_QUERY_OUTPUT_ATTR, 
//                         &app_ctx->output_attrs_landmark[i], sizeof(rknn_tensor_attr));
                        
//         if (ret != RKNN_SUCC)
//         {
//             printf("ERROR: rknn_query(RKNN_QUERY_OUTPUT_ATTR) for landmark output %d failed! ret=%d\n", i, ret);
//             goto cleanup_landmark;
//         }
...
//     {
//         printf("Landmark model is NCHW input fmt\n");
...
//     else
//     {
//         printf("Landmark model is NHWC input fmt\n");
...
// {
//     if (app_ctx->input_attrs_detect != NULL)
//     {
//         free(app_ctx->input_attrs_detect);
//         app_ctx->input_attrs_detect = NULL;
//     }
    
//     if (app_ctx->output_attrs_detect != NULL)
//     {
//         free(app_ctx->output_attrs_detect);
//         app_ctx->output_attrs_detect = NULL;
//     }
    
//     if (app_ctx->rknn_ctx_detect != 0)
//     {
//         rknn_destroy(app_ctx->rknn_ctx_detect);
//         app_ctx->rknn_ctx_detect = 0;
//     }
    
//     if (app_ctx->input_attrs_landmark != NULL)
//     {
//         free(app_ctx->input_attrs_landmark);
//         app_ctx->input_attrs_landmark = NULL;
//     }
    
//     if (app_ctx->output_attrs_landmark != NULL)
//     {
//         free(app_ctx->output_attrs_landmark);
//         app_ctx->output_attrs_landmark = NULL;
//     }
    
//     if (app_ctx->rknn_ctx_landmark != 0)
//     {
//         rknn_destroy(app_ctx->rknn_ctx_landmark);
//         app_ctx->rknn_ctx_landmark = 0;
//     }
    
//     return 0;
// }

// int inference_face_analyzer_models(rknn_app_context_t *app_ctx, image_buffer_t *src_img, 
//                                   face_analyzer_result_t *out_result)
// {
//     int ret = 0;
//     memset(out_result, 0, sizeof(face_analyzer_result_t));

//     image_buffer_t detect_img;
//     letterbox_t det_letter_box;
//     rknn_input detect_inputs[1];
//     rknn_output detect_outputs[app_ctx->io_num_detect.n_output];
    
//     memset(&detect_img, 0, sizeof(image_buffer_t));
//     memset(detect_inputs, 0, sizeof(detect_inputs));
//     memset(detect_outputs, 0, app_ctx->io_num_detect.n_output * sizeof(rknn_output));
//     memset(&det_letter_box, 0, sizeof(letterbox_t));

//     int bg_color = 114;
//     detect_img.width = app_ctx->model1_width;
//     detect_img.height = app_ctx->model1_height;
//     detect_img.format = IMAGE_FORMAT_RGB888;
//     detect_img.size = get_image_size(&detect_img);
//     detect_img.virt_addr = (unsigned char *)malloc(detect_img.size);
    
//     if (detect_img.virt_addr == NULL)
//     {
//         printf("ERROR: Failed to malloc buffer for detection input (%d bytes)!\n", (int)detect_img.size);
//         return -1;
//     }

//     ret = convert_image_with_letterbox(src_img, &detect_img, &det_letter_box, bg_color);
    
//     if (ret < 0)
//     {
//         printf("ERROR: convert_image_with_letterbox for detection failed! ret=%d\n", ret);
//         free(detect_img.virt_addr);
//         return -1;
//     }

//     detect_inputs[0].index = 0;
//     detect_inputs[0].type = RKNN_TENSOR_UINT8;
    
//     if (app_ctx->input_fmt_detect == RKNN_TENSOR_NCHW)
//     {
//         detect_inputs[0].fmt = RKNN_TENSOR_NCHW;
//         detect_inputs[0].size = app_ctx->model1_width * app_ctx->model1_height * app_ctx->model1_channel;
//     }
//     else
//     {
//         detect_inputs[0].fmt = RKNN_TENSOR_NHWC;
//         detect_inputs[0].size = app_ctx->model1_width * app_ctx->model1_height * app_ctx->model1_channel;
//     }
    
//     detect_inputs[0].buf = detect_img.virt_addr;
    
//     ret = rknn_inputs_set(app_ctx->rknn_ctx_detect, app_ctx->io_num_detect.n_input, detect_inputs);
    
//     if (ret < 0)
//     {
//         printf("ERROR: rknn_inputs_set for detection failed! ret=%d\n", ret);
//         free(detect_img.virt_addr);
//         return -1;
//     }
...
//     if (ret < 0)
//     {
//         printf("ERROR: rknn_run for detection failed! ret=%d\n", ret);
//         free(detect_img.virt_addr);
//         return -1;
//     }
...
//     for (int i = 0; i < app_ctx->io_num_detect.n_output; i++)
//     {
//         detect_outputs[i].index = i;
//         detect_outputs[i].want_float = 0;
//     }
    
//     ret = rknn_outputs_get(app_ctx->rknn_ctx_detect, app_ctx->io_num_detect.n_output, detect_outputs, NULL);
    
//     if (ret < 0)
//     {
//         printf("ERROR: rknn_outputs_get for detection failed! ret=%d\n", ret);
//         free(detect_img.virt_addr);
//         rknn_outputs_release(app_ctx->rknn_ctx_detect, app_ctx->io_num_detect.n_output, detect_outputs);
//         return -1;
//     }

//     std::vector<DetectionCandidate> detected_faces;
//     ret = post_process_face_detection(detect_outputs, app_ctx, &det_letter_box, 
...
//     if (ret < 0)
//     {
//         printf("ERROR: post_process_face_detection failed! ret=%d\n", ret);
//         return -1;
//     }
...
//     image_buffer_t landmark_img;
//     letterbox_t lmk_letter_box;
//     rknn_input landmark_inputs[1];
//     rknn_output landmark_outputs[app_ctx->io_num_landmark.n_output];
    
//     memset(&landmark_img, 0, sizeof(image_buffer_t));
//     memset(landmark_inputs, 0, sizeof(landmark_inputs));
//     memset(landmark_outputs, 0, app_ctx->io_num_landmark.n_output * sizeof(rknn_output));
//     memset(&lmk_letter_box, 0, sizeof(letterbox_t));

//     landmark_img.width = app_ctx->model2_width;
//     landmark_img.height = app_ctx->model2_height;
//     landmark_img.format = IMAGE_FORMAT_RGB888;
//     landmark_img.size = get_image_size(&landmark_img);
//     landmark_img.virt_addr = (unsigned char *)malloc(landmark_img.size);
    
//     if (landmark_img.virt_addr == NULL)
//     {
//         printf("ERROR: Failed to malloc buffer for landmark input (%d bytes)!\n", (int)landmark_img.size);
//         return -1;
//     }

//     for (int i = 0; i < num_faces_to_process; ++i)
//     {
//         const box_rect_t& face_box = detected_faces[i].box;
//         out_result->faces[i].box = face_box;
//         out_result->faces[i].score = detected_faces[i].score;
//         out_result->faces[i].landmarks_valid = false;

//         int center_x = (face_box.left + face_box.right) / 2;
//         int center_y = (face_box.top + face_box.bottom) / 2;
//         int box_w = face_box.right - face_box.left;
//         int box_h = face_box.bottom - face_box.top;
//         int crop_w = (int)(box_w * BOX_SCALE_X);
//         int crop_h = (int)(box_h * BOX_SCALE_Y);

//         box_rect_t crop_roi;
//         crop_roi.left = center_x - crop_w / 2;
//         crop_roi.top = center_y - crop_h / 2;
//         crop_roi.right = crop_roi.left + crop_w;
//         crop_roi.bottom = crop_roi.top + crop_h;

//         image_buffer_t cropped_face_img;
//         memset(&cropped_face_img, 0, sizeof(image_buffer_t));
//         cropped_face_img.virt_addr = NULL;
        
//         ret = crop_image(src_img, &cropped_face_img, crop_roi);
        
//         if (ret < 0)
//         {
//             printf("WARN: Failed to crop image for face %d, skipping landmarks.\n", i);
//             if (cropped_face_img.virt_addr) free(cropped_face_img.virt_addr);
//             continue;
//         }

//         memset(&lmk_letter_box, 0, sizeof(letterbox_t));
//         ret = convert_image_with_letterbox(&cropped_face_img, &landmark_img, &lmk_letter_box, bg_color);
        
//         if (cropped_face_img.virt_addr)
//         {
//             free(cropped_face_img.virt_addr);
//             cropped_face_img.virt_addr = NULL;
//         }
        
//         if (ret < 0)
//         {
//             printf("WARN: convert_image_with_letterbox for landmarks failed for face %d! ret=%d\n", i, ret);
//             continue;
//         }

//         landmark_inputs[0].index = 0;
//         landmark_inputs[0].type = RKNN_TENSOR_UINT8;
        
//         if (app_ctx->input_fmt_landmark == RKNN_TENSOR_NCHW)
//         {
//             landmark_inputs[0].fmt = RKNN_TENSOR_NCHW;
//             landmark_inputs[0].size = app_ctx->model2_width * app_ctx->model2_height * app_ctx->model2_channel;
//         }
//         else
//         {
//             landmark_inputs[0].fmt = RKNN_TENSOR_NHWC;
//             landmark_inputs[0].size = app_ctx->model2_width * app_ctx->model2_height * app_ctx->model2_channel;
//         }
        
//         landmark_inputs[0].buf = landmark_img.virt_addr;
        
//         ret = rknn_inputs_set(app_ctx->rknn_ctx_landmark, app_ctx->io_num_landmark.n_input, landmark_inputs);
        
//         if (ret < 0)
//         {
//             printf("WARN: rknn_inputs_set for landmarks failed for face %d! ret=%d\n", i, ret);
//             continue;
//         }
...
//         if (ret < 0)
//         {
//             printf("WARN: rknn_run for landmarks failed for face %d! ret=%d\n", i, ret);
//             continue;
//         }

//         for (int j = 0; j < app_ctx->io_num_landmark.n_output; j++)
//         {
//             landmark_outputs[j].index = j;
//             landmark_outputs[j].want_float = 0;
//         }
        
//         ret = rknn_outputs_get(app_ctx->rknn_ctx_landmark, app_ctx->io_num_landmark.n_output, 
//                               landmark_outputs, NULL);
                              
//         if (ret < 0)
//         {
//             printf("WARN: rknn_outputs_get for landmarks failed for face %d! ret=%d\n", i, ret);
//             rknn_outputs_release(app_ctx->rknn_ctx_landmark, app_ctx->io_num_landmark.n_output, landmark_outputs);
//             continue;
//         }

//         ret = post_process_face_landmarks(landmark_outputs[0], app_ctx, crop_roi, 
//                                          &lmk_letter_box, out_result->faces[i].landmarks);
                                         
//         rknn_outputs_release(app_ctx->rknn_ctx_landmark, app_ctx->io_num_landmark.n_output, landmark_outputs);
        
//         if (ret < 0)
//         {
//             printf("WARN: post_process_face_landmarks failed for face %d! ret=%d\n", i, ret);
//         }
//         else
//         {
//             out_result->faces[i].landmarks_valid = true;
//         }
//     }

//     if (landmark_img.virt_addr != NULL)
//     {
//         free(landmark_img.virt_addr);
//     }
...
static int post_process_face_detection(rknn_output outputs[], rknn_app_context_t *app_ctx, const letterbox_t *det_letter_box, int src_img_width, int src_img_height, std::vector<DetectionCandidate>& out_faces) {
...
static int post_process_face_landmarks(rknn_output landmark_output, rknn_app_context_t *app_ctx, const box_rect_t& crop_roi, const letterbox_t *lmk_letter_box, point_t out_landmarks[NUM_FACE_LANDMARKS]) {
...
static int post_process_iris_landmarks(rknn_output iris_outputs[], rknn_app_context_t *app_ctx, const box_rect_t& eye_crop_roi, const letterbox_t *iris_letter_box, point_t out_eye_contour[NUM_EYE_CONTOUR_LANDMARKS], point_t out_iris[NUM_IRIS_LANDMARKS]) {
...
cleanup_detect: if (app_ctx->rknn_ctx_detect != 0) rknn_destroy(app_ctx->rknn_ctx_detect); if (app_ctx->input_attrs_detect) free(app_ctx->input_attrs_detect); if (app_ctx->output_attrs_detect) free(app_ctx->output_attrs_detect); memset(app_ctx, 0, sizeof(rknn_app_context_t)); return -1;
}
...
int inference_face_analyzer_models(rknn_app_context_t *app_ctx, image_buffer_t *src_img, face_analyzer_result_t *out_result) {
