...
//     GstRTSPMediaFactory *factory_cam0 = gst_rtsp_media_factory_new();
//     gst_rtsp_media_factory_set_launch(factory_cam0, "( v4l2src device=/dev/video1 ! video/x-raw,width=1280,height=720 ! videoconvert ! queue ! mpph264enc bps=2000000 ! h264parse config-interval=-1 ! rtph264pay name=pay0 pt=96 )");
//     gst_rtsp_media_factory_set_shared(factory_cam0, TRUE);
//     gst_rtsp_mount_points_add_factory(mounts, "/cam0", factory_cam0);
//     std::cout << "[RTSP Thread] Stream ready at rtsp://<your_ip>:8554/cam0" << std::endl;

//     GstRTSPMediaFactory *factory_cam1 = gst_rtsp_media_factory_new();
//     gst_rtsp_media_factory_set_launch(factory_cam1, "( v4l2src device=/dev/video0 ! video/x-raw,width=1280,height=720 ! videoconvert ! queue ! mpph264enc bps=2000000 ! h264parse config-interval=-1 ! rtph264pay name=pay0 pt=96 )");
//     gst_rtsp_media_factory_set_shared(factory_cam1, TRUE);
//     gst_rtsp_mount_points_add_factory(mounts, "/cam1", factory_cam1);
...
std::atomic<uint8_t> g_rtsp_server_status(0);
std::atomic<uint8_t> g_active_mode_id(0);
// --- NEW: Global variable to hold the current resolution setting ---
std::atomic<uint8_t> g_resolution_setting(0); // Default to 0 (640x360)
...
#define GST_TYPE_DYNAMIC_MEDIA_FACTORY (dynamic_media_factory_get_type())
G_DECLARE_FINAL_TYPE(DynamicMediaFactory, dynamic_media_factory, DYNAMIC, MEDIA_FACTORY, GstRTSPMediaFactory)

struct _DynamicMediaFactory {
...
static GstElement *dynamic_media_factory_create_element(GstRTSPMediaFactory *factory, const GstRTSPUrl *url) {
    DynamicMediaFactory *self = (DynamicMediaFactory *)factory;
    gchar *launch_string = dynamic_media_factory_create_launch_string(self);
    GstElement *pipeline = gst_parse_launch(launch_string, NULL);
    g_free(launch_string);
    return pipeline;
}

static void dynamic_media_factory_init(DynamicMediaFactory *factory) {}

static void dynamic_media_factory_class_init(DynamicMediaFactoryClass *klass) {
    GstRTSPMediaFactoryClass *parent_class = GST_RTSP_MEDIA_FACTORY_CLASS(klass);
    parent_class->create_element = dynamic_media_factory_create_element;
}

// Helper function to create a new instance of our factory
static DynamicMediaFactory *dynamic_media_factory_new(const gchar *device_path) {
    DynamicMediaFactory *factory = (DynamicMediaFactory *)g_object_new(GST_TYPE_DYNAMIC_MEDIA_FACTORY, NULL);
    factory->device_path = g_strdup(device_path);
    return factory;
}
...
// // =========================================================================
// //  THREAD 5: COMMAND PROCESSING WORKER (ORIGINAL SIMPLE VERSION)
// // =========================================================================
void commandProcessingThread() {
    std::cout << "[Processing Thread] Worker thread started." << std::endl;
    while (!g_shutdown_request.load()) {
