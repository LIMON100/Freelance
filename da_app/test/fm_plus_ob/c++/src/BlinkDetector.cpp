...
// #include <iostream>
// #include <opencv2/imgproc.hpp> // Required for cv::putText
// #include <numeric>
// #include <chrono>

// float BlinkDetector::smoothEAR(std::deque<float>& earHistory, float newEAR) {
...
// void BlinkDetector::run(const cv::Mat& frame, const my::IrisLandmark& irisLandmarker) {
//     int h = frame.rows;
//     int w = frame.cols;
...
//     float leftEAR = calculateEAR(faceLandmarks, LEFT_EYE_POINTS, w, h);
//     float rightEAR = calculateEAR(faceLandmarks, RIGHT_EYE_POINTS, w, h);

//     leftEAR = smoothEAR(leftEARHistory, leftEAR);
//     rightEAR = smoothEAR(rightEARHistory, rightEAR);

//     // Check if either eye is closed
//     bool isEyeClosedNow = isEyeClosed(leftEAR) || isEyeClosed(rightEAR);

//     if (isEyeClosedNow && !wasEyeClosed) {
//          // Blink starts (either eye closes)
//         blinkStartTime = std::chrono::high_resolution_clock::now();
//         wasEyeClosed = true;
        
//     } else if (!isEyeClosedNow && wasEyeClosed) {
//         //Blink End
//         auto now = std::chrono::high_resolution_clock::now();
//         double blinkDuration = std::chrono::duration_cast<std::chrono::milliseconds>(now - blinkStartTime).count() / 1000.0;
//         blink_counter++;
...
// float BlinkDetector::calculateEAR(const std::vector<cv::Point>& landmarks, const std::vector<int>& eye_points, int w, int h) {
//     try {
...
// bool BlinkDetector::isEyeClosed(float ear, float threshold) {
