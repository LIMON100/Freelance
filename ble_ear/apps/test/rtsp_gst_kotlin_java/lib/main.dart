...
//   //             35,
//   //             "Driving",
//   //             ICON_PATH_DRIVING_INACTIVE,
//   //             ICON_PATH_DRIVING_ACTIVE,
//   //             _activeLeftButtonIndex == 0, () =>
//   //             _onLeftButtonPressed(0, 'CMD_MODE_DRIVE')),
//   //         _buildSideButton(
//   //             30,
//   //             185,
//   //             "Petrol",
//   //             ICON_PATH_PETROL_INACTIVE,
//   //             ICON_PATH_PETROL_ACTIVE,
//   //             _activeLeftButtonIndex == 1, () =>
//   //             _onLeftButtonPressed(1, 'CMD_MODE_PATROL')),
//   //         _buildSideButton(
//   //             30,
//   //             335,
//   //             "Recon",
//   //             ICON_PATH_RECON_INACTIVE,
//   //             ICON_PATH_RECON_ACTIVE,
//   //             _activeLeftButtonIndex == 2, () =>
//   //             _onLeftButtonPressed(2, 'CMD_MODE_RECON')),
//   //         _buildSideButton(
//   //             30,
//   //             485,
//   //             _isAutoAttackMode ? "Auto Attack" : "Manual Attack",
//   //             _isAutoAttackMode
//   //                 ? ICON_PATH_AUTO_ATTACK_INACTIVE
//   //                 : ICON_PATH_MANUAL_ATTACK_INACTIVE,
//   //             _isAutoAttackMode
//   //                 ? ICON_PATH_AUTO_ATTACK_ACTIVE
//   //                 : ICON_PATH_MANUAL_ATTACK_ACTIVE,
...
//   //             635,
//   //             "Drone",
//   //             ICON_PATH_DRONE_INACTIVE,
//   //             ICON_PATH_DRONE_ACTIVE,
//   //             _activeLeftButtonIndex == 4, () =>
//   //             _onLeftButtonPressed(4, 'CMD_MODE_DRONE')),
//   //         _buildSideButton(
//   //             30,
//   //             785,
//   //             "Return",
//   //             ICON_PATH_RETURN_INACTIVE,
//   //             ICON_PATH_RETURN_ACTIVE,
//   //             _activeLeftButtonIndex == 5, () =>
//   //             _onLeftButtonPressed(5, 'CMD_RETUR')),
//   //         _buildSideButton(
//   //             1690,
//   //             30,
//   //             "Attack View",
//   //             ICON_PATH_ATTACK_VIEW_INACTIVE,
//   //             ICON_PATH_ATTACK_VIEW_ACTIVE,
//   //             _activeRightButtonIndex == 0, () => _onRightButtonPressed(0)),
//   //         _buildSideButton(
//   //             1690,
//   //             250,
//   //             "Top View",
//   //             ICON_PATH_TOP_VIEW_INACTIVE,
//   //             ICON_PATH_TOP_VIEW_ACTIVE,
//   //             _activeRightButtonIndex == 1, () => _onRightButtonPressed(1)),
//   //         _buildSideButton(
//   //             1690,
//   //             470,
//   //             "3d View",
//   //             ICON_PATH_3D_VIEW_INACTIVE,
//   //             ICON_PATH_3D_VIEW_ACTIVE,
//   //             _activeRightButtonIndex == 2, () => _onRightButtonPressed(2)),
//   //         _buildSideButton(
//   //             1690,
//   //             720,
//   //             "Setting",
//   //             ICON_PATH_SETTINGS,
//   //             ICON_PATH_SETTINGS,
//   //             false, () => _navigateToSettings()),
//   //         _buildDirectionalButton(
//   //             890,
//   //             30,
//   //             _isForwardPressed,
//   //             ICON_PATH_FORWARD_INACTIVE,
//   //             ICON_PATH_FORWARD_ACTIVE,
//   //             'CMD_FORWARD', () =>
...
//   //             820,
//   //             _isBackPressed,
//   //             ICON_PATH_BACKWARD_INACTIVE,
//   //             ICON_PATH_BACKWARD_ACTIVE,
//   //             'CMD_BACKWARD', () => setState(() => _isBackPressed = true), () =>
//   //             setState(() => _isBackPressed = false)),
//   //         _buildDirectionalButton(
//   //             260,
//   //             405,
//   //             _isLeftPressed,
//   //             ICON_PATH_LEFT_INACTIVE,
//   //             ICON_PATH_LEFT_ACTIVE,
//   //             'CMD_LEFT', () => setState(() => _isLeftPressed = true), () =>
//   //             setState(() => _isLeftPressed = false)),
//   //         _buildDirectionalButton(
//   //             1560,
//   //             405,
//   //             _isRightPressed,
//   //             ICON_PATH_RIGHT_INACTIVE,
//   //             ICON_PATH_RIGHT_ACTIVE,
//   //             'CMD_RIGHT', () => setState(() => _isRightPressed = true), () =>
...
import 'package:flutter/material.dart';
import 'package:flutter/services.dart';
// We no longer need the VLC player import
// import 'package:flutter_vlc_player/flutter_vlc_player.dart';
import 'package:google_fonts/google_fonts.dart';
...
class _HomePageState extends State<HomePage> {
  // --- STATE MANAGEMENT: VLC REPLACED WITH GSTREAMER ---

  // VlcPlayerController? _vlcPlayerController; // REMOVED
  MethodChannel? _gstreamerChannel; // ADDED: The channel to talk to our native view.

  // All UI and logic state variables remain the same
  List<String> _cameraUrls = [
    'rtsp://192.168.0.158:8554/cam0',
    'rtsp://192.168.0.158:8554/cam1', // Assuming cam1 for the second URL
    'rtsp://192.168.0.158:8554/cam2', // Assuming cam2 for the third URL
  ];
  int _currentCameraIndex = -1;
  int _activeLeftButtonIndex = 0;
  int _activeRightButtonIndex = 0;
  ...
  // New state for GStreamer player UI
  ...
  void initState() {
    super.initState();
    // The initial camera switch will now happen in `_onPlatformViewCreated`
    // because we need the MethodChannel to be ready first.
  }

  @override
  void dispose() {
    // The native GStreamer view handles its own disposal when the widget is removed.
    // We can add a manual 'finalize' call if needed, but this is often sufficient.
    super.dispose();
  }

  // --- NATIVE VIEW CALLBACK ---
  void _onPlatformViewCreated(int id) {
    // This is called when the native AndroidView is ready.
    // We create the channel here.
    _gstreamerChannel = MethodChannel('gstreamer_channel_$id');

    // Now that the view and channel are ready, switch to the first camera.
  ...
  // --- All backend logic functions remain unchanged, EXCEPT _switchCamera ---
  Future<void> _sendCommand(String command) async {
    try {
      final socket = await Socket.connect(ROBOT_IP_ADDRESS, ROBOT_COMMAND_PORT,
          timeout: const Duration(seconds: 5));
  ...
  Future<void> _switchCamera(int index) async {
    // If the native view isn't ready yet, do nothing.
    if (_gstreamerChannel == null) return;

    // Prevent reloading the same stream if the button is tapped again.
    if (index == _currentCameraIndex && !_hasError) return;
    ...
    try {
      // Send the 'startStream' command to our native GStreamer code.
      await _gstreamerChannel!.invokeMethod('startStream', {'url': _cameraUrls[index]});

      print("GStreamer native method 'startStream' invoked for URL: ${_cameraUrls[index]}");

      // Give GStreamer a moment to connect, then hide the loader.
      ...
      print("Failed to start GStreamer stream: '${e.message}'.");
      if (mounted) {
        setState(() {
          _hasError = true;
          _isLoading = false;
  ...
  Future<void> _navigateToSettings() async {
    ...
    if (newUrls != null) {
      setState(() => _cameraUrls = newUrls);
      // After returning from settings, just switch to the current camera index
      // which will reload the stream with the potentially new URL.
      _switchCamera(_currentCameraIndex);
    }
  }

  // All other helper functions for dialogs and button presses are IDENTICAL.
  ...
  @override
  Widget build(BuildContext context) {
    // The main build method remains unchanged.
    // It will now call our new `buildPlayerWidget`.
    final screenWidth = MediaQuery.of(context).size.width;
    final screenHeight = MediaQuery.of(context).size.height;
    final widthScale = screenWidth / 1920.0;
    final heightScale = screenHeight / 1080.0;
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          Positioned.fill(child: buildPlayerWidget()), // This now builds the GStreamer view
          _buildLeftButton(30, 35, "Driving", ICON_PATH_DRIVING_INACTIVE, ICON_PATH_DRIVING_ACTIVE, _activeLeftButtonIndex == 0, () => _onLeftButtonPressed(0, 'CMD_MODE_DRIVE')),
          _buildLeftButton(30, 185, "Petrol", ICON_PATH_PETROL_INACTIVE, ICON_PATH_PETROL_ACTIVE, _activeLeftButtonIndex == 1, () => _onLeftButtonPressed(1, 'CMD_MODE_PATROL')),
          _buildLeftButton(30, 335, "Recon", ICON_PATH_RECON_INACTIVE, ICON_PATH_RECON_ACTIVE, _activeLeftButtonIndex == 2, () => _onLeftButtonPressed(2, 'CMD_MODE_RECON')),
          _buildLeftButton(30, 485, _isAutoAttackMode ? "Auto Attack" : "Manual Attack", _isAutoAttackMode ? ICON_PATH_AUTO_ATTACK_INACTIVE : ICON_PATH_MANUAL_ATTACK_INACTIVE, _isAutoAttackMode ? ICON_PATH_AUTO_ATTACK_ACTIVE : ICON_PATH_MANUAL_ATTACK_ACTIVE, _activeLeftButtonIndex == 3, () { setState(() => _activeLeftButtonIndex = 3); _handleManualAutoAttackToggle(); }),
  ...
  void _onRightButtonPressed(int index) {
    setState(() => _activeRightButtonIndex = index);
    // This now calls our new GStreamer-based camera switcher.
    _switchCamera(index);
  }

  // --- THIS WIDGET IS ALSO REPLACED ---
  Widget buildPlayerWidget() {
    return Stack(
      alignment: Alignment.center,
      children: [
        // The base layer is our native GStreamer view.
        AndroidView(
          viewType: 'gstreamer_view',
          onPlatformViewCreated: _onPlatformViewCreated,
        ),

        // Show a loading indicator on top while connecting.
        if (_isLoading)
          const CircularProgressIndicator(color: Colors.white),

        // Show an error overlay if the stream fails.
        if (_hasError)
          Container(
            ...
            child: Center(
              child: Column(
                ...
                children: [
                  ...
                  const Text('Error: Could not load stream.', style: TextStyle(color: Colors.white, fontSize: 16)),
                  const SizedBox(height: 16),
                  ElevatedButton(
                      onPressed: () => _switchCamera(_currentCameraIndex), // Retry logic
  ...
  // ALL OTHER _build... WIDGETS ARE UNCHANGED AND PERFECT AS THEY ARE.
  // ... ( _buildLeftButton, _buildRightButton, _buildDirectionalButton, _buildBottomBar, etc.)
  Widget _buildLeftButton(double left, double top, String label, String inactiveIcon, String activeIcon, bool isActive, VoidCallback onPressed) {
    final screenWidth = MediaQuery.of(context).size.width;
    final screenHeight = MediaQuery.of(context).size.height;
    final widthScale = screenWidth / 1920.0;
