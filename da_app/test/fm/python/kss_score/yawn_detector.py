...
class YawnDetector:
    ...
    def calculate_mar(self, landmarks, w, h):
        ...
        mouth_top = landmarks.landmark[13]  # Top lip
        mouth_bottom = landmarks.landmark[14]  # Bottom lip

        # Convert to pixel coordinates
        x1, y1 = int(mouth_left.x * w), int(mouth_left.y * h)  # Left corner
        x2, y2 = int(mouth_right.x * w), int(mouth_right.y * h)  # Right corner
        x3, y3 = int(mouth_top.x * w), int(mouth_top.y * h)  # Top lip
        x4, y4 = int(mouth_bottom.x * w), int(mouth_bottom.y * h)  # Bottom lip

        # Calculate mouth width and height
        mouth_width = math.hypot(x2 - x1, y2 - y1)  # Distance between left and right corners
        mouth_height = math.hypot(x4 - x3, y4 - y3)  # Distance between top and bottom lips
