...
import 'package:flutter/services.dart';
import 'package:flutter_vlc_player/flutter_vlc_player.dart';
// --- FONT UPDATE: Removed the google_fonts package import ---
// import 'package:google_fonts/google_fonts.dart';
import 'package:rtest1/settings_service.dart';
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      theme: ThemeData(
        primarySwatch: Colors.blue,
        // --- FONT UPDATE: Removed the Google Fonts theme. The app will use the local font specified in Text widgets. ---
        // textTheme: GoogleFonts.rajdhaniTextTheme(),
      ),
      home: const SplashScreen(),
    );
...
class _HomePageState extends State<HomePage> {
          ...
          child: Container(width: 500, padding: const EdgeInsets.all(24.0),
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              Row(children: [
                Image.asset(iconPath, height: 40, width: 40),
                const SizedBox(width: 16),
                // --- FONT UPDATE: Replaced GoogleFonts with TextStyle ---
                Text(title, style: TextStyle(fontFamily: 'NotoSans', fontSize: 28,
                    fontWeight: FontWeight.bold,
                    color: titleColor))
              ]),
              const SizedBox(height: 24),
              ...
              Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
                ...
                TextButton(
                    onPressed: () => Navigator.of(dialogContext).pop(false),
                    child: Text('Cancel', style: TextStyle(fontFamily: 'NotoSans',
                        fontSize: 24, fontWeight: FontWeight.bold, color: Colors
                            .grey.shade700))),
                SizedBox(height: 40,
                    child: VerticalDivider(
                ...
                TextButton(
                    onPressed: () => Navigator.of(dialogContext).pop(true),
                    child: Text('OK', style: TextStyle(fontFamily: 'NotoSans',
                        fontSize: 24, fontWeight: FontWeight.bold, color: Colors
                            .blue.shade700))),
              ],
              ),
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          _buildLeftButton(30, 185, "Petrol", ICON_PATH_PETROL_INACTIVE, ICON_PATH_PETROL_ACTIVE, _activeLeftButtonIndex == 1, () => _onLeftButtonPressed(1, 2)),
          _buildLeftButton(30, 335, "Recon", ICON_PATH_RECON_INACTIVE, ICON_PATH_RECON_ACTIVE, _activeLeftButtonIndex == 2, () => _onLeftButtonPressed(2, 2)),
          _buildLeftButton(30, 485, _isAutoAttackMode ? "Auto Attack" : "Manual Attack", _isAutoAttackMode ? ICON_PATH_AUTO_ATTACK_INACTIVE : ICON_PATH_MANUAL_ATTACK_INACTIVE, _isAutoAttackMode ? ICON_PATH_AUTO_ATTACK_ACTIVE : ICON_PATH_MANUAL_ATTACK_ACTIVE, _activeLeftButtonIndex == 3, () {
            setState(() {
              _activeLeftButtonIndex = 3;
              _isManualTouchModeEnabled = true;
            });
    ...
    if (_errorMessage != null) {
      return Center(
          child: Column( mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            const SizedBox(height: 20),
            ElevatedButton( onPressed: () => _switchCamera(_currentCameraIndex),
    ...
    return GestureDetector(
      ...
      onTapUp: (details) {
        if (_isManualTouchModeEnabled) {
          final RenderBox? box = _playerKey.currentContext?.findRenderObject() as RenderBox?;
          if (box == null) return;
  ...
  Widget _buildLeftButton(double left, double top, String label, String inactiveIcon, String activeIcon, bool isActive, VoidCallback onPressed) {
    ...
    final widthScale = screenWidth / 1920.0;
    final heightScale = screenHeight / 1080.0;
    final containerColor = isActive ? const Color(0xFFc32121) : Colors.black.withOpacity(0.6);
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 5 * heightScale),
              // --- FONT UPDATE: Replaced GoogleFonts with TextStyle ---
              Text(label, textAlign: TextAlign.center, style: TextStyle(fontFamily: 'NotoSans', fontSize: 24 * heightScale, fontWeight: FontWeight.bold, color: const Color(0xFFFFFFFF))),
            ],
          ),
        ),
      ),
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 8 * heightScale),
              // --- FONT UPDATE: Replaced GoogleFonts with TextStyle ---
              Text(label, textAlign: TextAlign.center, style: TextStyle(fontFamily: 'NotoSans', fontSize: 26 * heightScale, fontWeight: FontWeight.bold, color: const Color(0xFFFFFFFF))),
            ],
          ),
        ),
      ),
  ...
  Widget _buildBottomBar() {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final List<Widget> leftCluster = [
          _buildBottomBarButton( "ATTACK", _isAttackModeOn ? ICON_PATH_ATTACK_of : ICON_PATH_ATTACK,
            _isAttackModeOn ? _attackActiveColors : _attackInactiveColors, () async {
              if (!_isAttackModeOn) {
                final proceed = await _showCustomConfirmationDialog(context: context, iconPath: ICON_PATH_ATTACK, title: 'DANGERS', titleColor: Colors.red, content: 'Do you want to proceed the command?');
                if (proceed) {
                  setState(() {
          ...
          SizedBox(width: 12 * widthScale),
          _buildWideBottomBarButton( _isStarted ? "STOP" : "START", _isStarted ? ICON_PATH_STOP : ICON_PATH_START,
              [const Color(0xff25a625), const Color(0xff127812)], () {
                setState(() {
                  _isStarted = !_isStarted;
                  _isGunTriggerPressed = _isStarted;
          ...
          SizedBox(width: 12 * widthScale),
          _buildBottomBarButton("", ICON_PATH_PLUS, [const Color(0xffc0c0c0), const Color(0xffa0a0a0)], () { /* Zoom logic */ }),
          SizedBox(width: 12 * widthScale),
          _buildBottomBarButton("", ICON_PATH_MINUS, [const Color(0xffc0c0c0), const Color(0xffa0a0a0)], () { /* Zoom logic */ }),
        ];
        ...
        final List<Widget> middleCluster = [
          Row(
            ...
            children: [
              // --- FONT UPDATE: Replaced GoogleFonts with TextStyle ---
              Text("0", style: TextStyle(fontFamily: 'NotoSans', fontSize: 80, fontWeight: FontWeight.bold, color: Colors.white)),
              SizedBox(width: 8 * widthScale),
              // --- FONT UPDATE: Replaced GoogleFonts with TextStyle ---
              Text("Km/h", style: TextStyle(fontFamily: 'NotoSans', fontSize: 36, fontWeight: FontWeight.w500, color: Colors.white)),
            ],
          ),
          SizedBox(width: 20 * widthScale),
          Image.asset(ICON_PATH_WIFI, height: 40),
        ...
        if (constraints.maxWidth < scrollBreakpoint) {
          return SingleChildScrollView(
            ...
            child: Row( children: [ ...leftCluster, SizedBox(width: 100 * widthScale), ...middleCluster, SizedBox(width: 100 * widthScale), ...rightCluster, ], ),
          );
        } else {
          return Row( children: [ ...leftCluster, const Spacer(), ...middleCluster, const Spacer(), ...rightCluster, ], );
        }
      },
    );
  }
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row( mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.center, children: [
          ...
          const SizedBox(width: 12),
          // --- FONT UPDATE: Replaced GoogleFonts with TextStyle ---
          Text(label, style: TextStyle(fontFamily: 'NotoSans', fontSize: 36 * heightScale, fontWeight: FontWeight.bold, color: Colors.white)),
        ],
        ),
      ),
    );
