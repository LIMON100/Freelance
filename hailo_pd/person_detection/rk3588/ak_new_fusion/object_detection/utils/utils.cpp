...
#include "calibration.hpp" // Or wherever you defined the structs
bool loadHawkEyeCalibration(const std::string& calib_dir, HawkEyeCalibration& calib);

// In utils.cpp (implementation)
...
bool loadHawkEyeCalibration(const std::string& calib_dir, HawkEyeCalibration& calib) {
    try {
        ...
        std::ifstream f_it((calib_dir + "/intrinsics_thermal.json"));
        json it = json::parse(f_it);
        for (int i = 0; i < 3; ++i) for (int j = 0; j < 3; ++j) calib.thermal_intrinsics.K.at<double>(i, j) = it["K"][i][j];
        for (int i = 0; i < 5; ++i) calib.thermal_intrinsics.dist.at<double>(0, i) = it["dist"][i];

        // Load Extrinsics (Z_T means Thermal-to-Zoom)
        std::ifstream f_ex((calib_dir + "/extrinsics.json"));
        json ex = json::parse(f_ex);
        for (int i = 0; i < 3; ++i) for (int j = 0; j < 3; ++j) calib.thermal_to_zoom_extrinsics.R.at<double>(i, j) = ex["Z_T"]["R"][i][j];
        for (int i = 0; i < 3; ++i) calib.thermal_to_zoom_extrinsics.t.at<double>(i, 0) = ex["Z_T"]["t"][i];

        // Load Zoom LUT
        std::ifstream f_iz((calib_dir + "/intrinsics_zoom.json"));
        json iz = json::parse(f_iz);
        calib.zoom_lut_grid.zoom_bins = iz["zoom_bins"].get<std::vector<double>>();
        calib.zoom_lut_grid.focus_bins = iz["focus_bins"].get<std::vector<double>>();
        calib.zoom_lut_params.fCount = calib.zoom_lut_grid.focus_bins.size();
        calib.zoom_lut_params.zCount = calib.zoom_lut_grid.zoom_bins.size();
        
        // Helper to flatten 2D JSON array to 1D std::vector
        auto flatten = [](const json& j_arr) {
            std::vector<double> vec;
            for (const auto& row : j_arr) {
        ...
        calib.zoom_lut_params.fx = flatten(iz["fx"]);
        calib.zoom_lut_params.fy = flatten(iz["fy"]);
        // ... load cx, cy, k1, k2, etc. in the same way
        
    } catch (const std::exception& e) {
        std::cerr << "Failed to load or parse calibration JSONs: " << e.what() << std::endl;
        return false;
    }
    return true;
}
