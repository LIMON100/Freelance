...
// #pragma pack(push, 1)
// // Unified 13-byte Command Struct
...
//     const char* rpi_pipeline = 
//         "( libcamerasrc ! videoconvert ! x264enc tune=zerolatency bitrate=2000000 ! rtph264pay name=pay0 pt=96 )";

//     // --- Media Factory for /cam0 ---
//     GstRTSPMediaFactory *factory_cam0 = gst_rtsp_media_factory_new();
//     gst_rtsp_media_factory_set_launch(factory_cam0, rpi_pipeline);
...
//     // --- Media Factory for /cam1 (using a test source as a placeholder) ---
//     GstRTSPMediaFactory *factory_cam1 = gst_rtsp_media_factory_new();
//     gst_rtsp_media_factory_set_launch(factory_cam1, 
//         "( videotestsrc ! videoconvert ! x264enc tune=zerolatency ! rtph264pay name=pay0 pt=96 )"
//     );
...
//     // --- Media Factory for /cam2 (using a test source) ---
...
#include <sys/socket.h>
#include <netinet/in.h>
#include <chrono> // NEW: Needed for sleep_for
...
// =========================================================================
//  NEW: DEDICATED COMMAND PROCESSING THREAD
// =========================================================================
void commandProcessingThread() {
    ...
    while (true) {
        ...
        // Briefly lock the mutex just to check and grab a command
        {
            std::lock_guard<std::mutex> lock(command_queue_mutex);
            if (!command_queue.empty()) {
                command = command_queue.front();
        ...
        if (command_found) {
            // Now we can do slow operations like printing without blocking anything else.
            // This is also where you would put your motor control logic.
            ...
            // If the queue is empty, sleep for a short time to prevent this thread
            // from using 100% CPU.
...
// =========================================================================
//  DELETED: The check_for_commands function is no longer needed.
// =========================================================================
...
int main(int argc, char *argv[]) {
    ...
    // MODIFIED: Start all three threads
    std::thread cmd_thread(commandServerThread);
    std::thread touch_thread(touchServerThread);
    std::thread worker_thread(commandProcessingThread); // Start the new worker thread
    ...
    GstRTSPMediaFactory *factory_cam2 = gst_rtsp_media_factory_new();
    gst_rtsp_media_factory_set_launch(factory_cam2, 
        "( videotestsrc pattern=ball ! videoconvert ! x264enc tune=zerolatency ! rtph264pay name=pay0 pt=96 )"
    );
    gst_rtsp_media_factory_set_shared(factory_cam2, TRUE);
    gst_rtsp_mount_points_add_factory(mounts, "/cam2", factory_cam2);
    std::cout << "[Main Thread] Stream ready at rtsp://<your_ip>:8554/cam2" << std::endl;
    ...
    // DELETED: We no longer schedule work on the GStreamer main loop.
    // g_timeout_add(100, (GSourceFunc)check_for_commands, NULL);
    ...
    // Cleanup
    // MODIFIED: Join all three threads on exit
    cmd_thread.join();
    touch_thread.join();
    worker_thread.join();
