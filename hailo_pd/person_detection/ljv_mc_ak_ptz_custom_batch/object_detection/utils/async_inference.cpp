...
// File: async_inference.cpp
void AsyncModelInfer::infer_batch(
    const std::vector<std::shared_ptr<cv::Mat>>& input_batch,
    std::function<void(const hailort::AsyncInferCompletionInfo&,
                       const std::vector<std::pair<uint8_t*, hailo_vstream_info_t>>&,
                       const std::vector<std::shared_ptr<uint8_t>>&)> callback) {
    
    if (input_batch.size() > m_batch_size) {
        std::cerr << "Warning: Input batch size " << input_batch.size() 
                  << " exceeds configured batch size " << m_batch_size << std::endl;
    }
    
    std::vector<std::shared_ptr<cv::Mat>> input_guards;
    
    // Process each frame in the batch
    for (const auto& input_data : input_batch) {
