...
class CustomerProfileController extends GetxController {
  ...
  var fullName = TextEditingController();
  var phoneNumber = TextEditingController();
  var selectedCountryCode=RxString('');
  var pinCode = TextEditingController();
  var otpCon = TextEditingController();
  var bio = TextEditingController();
  RxList<String> countryList = RxList([]);
  ...
  void onInit() {
    // TODO: implement onInit
    initProfile();
    Get.lazyPut(()=> CustomerRequestState());
    super.onInit();
  }
  ...
  void initProfile() {
    ...
    pinCode.text = auth.value.pincode ?? "";
    bio.text = auth.value.bio ?? "";
    selectedCountryCode.value=auth.value.contactNumber.toString().substring(0, auth.value.contactNumber.indexOf(RegExp(r'\d')));
  ...
  Future<void> updateProfile() async {
    ...
    if (selectedCountryCode.value.isEmpty) {
      Tools.ShowErrorMessage("Please select country code");
      return;
    }
    if (selectedCountry.value.isEmpty) {
      Tools.ShowErrorMessage("Please select country");
    ...
    UserModel userModel = UserModel(
      ...
      pincode: pinCode.value.text,
      bio: bio.value.text,
      contactNumber:  selectedCountryCode.value+phoneNumber.value.text,
      joinedOn: auth.value.joinedOn,
      updatedOn: DateTime.now().microsecondsSinceEpoch,
      // dateOfBirth: selectedDob.toString(),
      // dateOfBirth: DateFormat('yyyy-MM-dd').format(selectedDob.value!),
