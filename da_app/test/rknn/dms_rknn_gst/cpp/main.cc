...
//         std::cerr << "Failed to create input pipeline: " << (error ? error->message : "Unknown error") << std::endl;
...
//         // --- Preprocess the Image for Better Detection (Histogram Equalization) ---
//         cv::Mat temp_frame(src_image.height, src_image.width, CV_8UC3, src_image.virt_addr, src_image.width_stride);
//         cv::Mat gray_frame, equalized_frame;
//         cv::cvtColor(temp_frame, gray_frame, cv::COLOR_BGR2GRAY); // Convert to grayscale
//         cv::equalizeHist(gray_frame, equalized_frame); // Apply histogram equalization
//         cv::cvtColor(equalized_frame, temp_frame, cv::COLOR_GRAY2BGR); // Convert back to BGR

//         // --- Run Face Analysis on Full Frame ---
//         ret = inference_face_analyzer(&app_ctx.face_ctx, &src_image, &face_results);
//         if (ret != 0) {
//             printf("Face analyzer inference failed, ret=%d\n", ret);
//             // Handle error if necessary
//         }

//         // --- Define Fixed ROI to Focus on the Right Side (Front Passenger) ---
//         int roi_left = 960;  // Start from the middle of a 1920x1080 image
//         int roi_top = 0;
//         int roi_right = 1920; // Cover the right half
//         int roi_bottom = 648; // 1080 * 0.6
//         int roi_width = roi_right - roi_left;
//         int roi_height = roi_bottom - roi_top;

//         // --- Find Front Passenger's Face (Rightmost Face in ROI) ---
//         int passenger_face_idx = -1;
//         float max_center_x = std::numeric_limits<float>::min();
//         for (int i = 0; i < face_results.count; ++i) {
//             face_object_t* face = &face_results.faces[i];
//             float center_x = (face->box.left + face->box.right) / 2.0f;
//             float center_y = (face->box.top + face->box.bottom) / 2.0f;
//             // Check if face center is within the Fixed ROI (right side)
...
//         bool use_cropped_region = false;
//         int cropped_left = 0, cropped_top = 0, cropped_right = 0, cropped_bottom = 0; // For drawing the cropped region

//         if (passenger_face_idx >= 0) {
//             // Front passenger's face detected, crop around the face
//             face_object_t* passenger_face = &face_results.faces[passenger_face_idx];
//             // Expand the bounding box
//             int box_width = passenger_face->box.right - passenger_face->box.left;
//             int box_height = passenger_face->box.bottom - passenger_face->box.top;
//             int new_width = static_cast<int>(box_width * 1.5);
//             int new_height = static_cast<int>(box_height * 1.2 + box_height); // Upward + downward
//             int new_left = passenger_face->box.left - (new_width - box_width) / 2;
//             int new_right = passenger_face->box.right + (new_width - box_width) / 2;
//             int new_top = passenger_face->box.top - static_cast<int>(box_height * 0.2);
//             int new_bottom = passenger_face->box.bottom + box_height;

//             // Ensure bounds are within image dimensions
//             new_left = std::max(0, new_left);
//             new_right = std::min(static_cast<int>(src_image.width), new_right);
//             new_top = std::max(0, new_top);
//             new_bottom = std::min(static_cast<int>(src_image.height), new_bottom);

//             // Store cropped region coordinates for drawing
//             cropped_left = new_left;
//             cropped_right = new_right;
//             cropped_top = new_top;
//             cropped_bottom = new_bottom;

//             // Update dimensions after clipping
//             new_width = new_right - new_left;
//             new_height = new_bottom - new_top;

//             // Allocate memory for the cropped region
//             input_image.width = new_width;
//             input_image.height = new_height;
//             input_image.width_stride = new_width * 3; // Assuming 3 channels (BGR/RGB)
//             input_image.height_stride = new_height;
//             input_image.format = src_image.format;
//             input_image.size = new_width * new_height * 3;
//             input_image.virt_addr = (unsigned char*)malloc(input_image.size);
//             if (!input_image.virt_addr) {
//                 printf("ERROR: Failed to allocate memory for cropped region.\n");
//                 // Fallback to Fixed ROI
//                 passenger_face_idx = -1;
//             } else {
//                 // Copy the cropped region
//                 for (int y = 0; y < new_height; ++y) {
//                     unsigned char* src_ptr = src_image.virt_addr + ((new_top + y) * src_image.width_stride + new_left * 3);
//                     unsigned char* dst_ptr = input_image.virt_addr + (y * input_image.width_stride);
//                     memcpy(dst_ptr, src_ptr, new_width * 3);
//                 }
...
//             // Fallback to Fixed ROI (right side) if no front passenger face is detected
//             input_image.width = roi_width;
//             input_image.height = roi_height;
//             input_image.width_stride = roi_width * 3;
//             input_image.height_stride = roi_height;
//             input_image.format = src_image.format;
//             input_image.size = roi_width * roi_height * 3;
//             input_image.virt_addr = (unsigned char*)malloc(input_image.size);
//             if (!input_image.virt_addr) {
//                 printf("ERROR: Failed to allocate memory for Fixed ROI.\n");
//                 gst_buffer_unmap(gst_buffer, &map_info);
//                 gst_sample_unref(sample);
//                 continue;
//             }
//             // Copy the Fixed ROI (right side)
//             for (int y = 0; y < roi_height; ++y) {
//                 unsigned char* src_ptr = src_image.virt_addr + ((roi_top + y) * src_image.width_stride + roi_left * 3);
//                 unsigned char* dst_ptr = input_image.virt_addr + (y * input_image.width_stride);
//                 memcpy(dst_ptr, src_ptr, roi_width * 3);
//             }
//         }
...
//         resized_image.width = 640;
//         resized_image.height = 640;
//         resized_image.width_stride = 640 * 3;
//         resized_image.height_stride = 640;
//         resized_image.format = input_image.format;
//         resized_image.size = 640 * 640 * 3;
//         resized_image.virt_addr = (unsigned char*)malloc(resized_image.size);
//         if (!resized_image.virt_addr) {
...
//         float scale = std::min(640.0f / input_image.width, 640.0f / input_image.height);
//         int scaled_width = static_cast<int>(input_image.width * scale);
//         int scaled_height = static_cast<int>(input_image.height * scale);
//         int pad_x = (640 - scaled_width) / 2;
//         int pad_y = (640 - scaled_height) / 2;

//         // Resize using OpenCV
//         cv::Mat src(input_image.height, input_image.width, CV_8UC3, input_image.virt_addr, input_image.width_stride);
//         cv::Mat dst(640, 640, CV_8UC3, resized_image.virt_addr, resized_image.width_stride);
//         dst.setTo(cv::Scalar(0, 0, 0)); // Fill with black
//         cv::Mat resized;
//         cv::resize(src, resized, cv::Size(scaled_width, scaled_height), 0, 0, cv::INTER_LINEAR);
//         // Copy resized image to the padded destination
//         resized.copyTo(dst(cv::Rect(pad_x, pad_y, scaled_width, scaled_height)));

//         // Free the intermediate image
//         free(input_image.virt_addr);
...
//                 if (eyes_valid_this_frame) consecutive_valid_eyes_frames++;
//                 else consecutive_valid_eyes_frames = 0;
...
//                         if (lS<EAR_STDDEV_THRESHOLD && rS<EAR_STDDEV_THRESHOLD) consecutive_stable_ear_frames++;
...
//                         if(mS<MOUTH_DIST_STDDEV_THRESHOLD) consecutive_stable_mouth_frames++;
...
//                         calib_right_ears.clear();
//                         calib_mouth_dists.clear();
//                         printf("WARN: Calibration time expired, criteria not met (H:%d, EV:%d, ES:%d, MS:%d). Retrying.\n", head_pose_calibrated, eyes_consistently_valid, ear_calibrated, mouth_calibrated);
//                     }
//                 }
//                 for (int i = 0; i < face_results.count; ++i) {
//                     draw_rectangle(&src_image, face_results.faces[i].box.left, face_results.faces[i].box.top, face_results.faces[i].box.right - face_results.faces[i].box.left, face_results.faces[i].box.bottom - face_results.faces[i].box.top, COLOR_YELLOW, 2);
//                 }
...
//         } else { // Normal Processing Phase
//             // --- Run Face Analysis on Cropped/Resized Region ---
//             face_analyzer_result_t cropped_face_results;
//             memset(&cropped_face_results, 0, sizeof(cropped_face_results));
//             ret = inference_face_analyzer(&app_ctx.face_ctx, &resized_image, &cropped_face_results);
//             if (ret != 0) {
//                 printf("Face analyzer inference on cropped region failed, ret=%d\n", ret);
//             }

//             // --- Queue Frame for YOLO ---
//             auto yolo_input_image = std::make_shared<image_buffer_t>();
//             yolo_input_image->size = resized_image.size;
//             yolo_input_image->virt_addr = (unsigned char*)malloc(yolo_input_image->size);
//             if (yolo_input_image->virt_addr) {
//                 memcpy(yolo_input_image->virt_addr, resized_image.virt_addr, resized_image.size);
//                 yolo_input_image->width = resized_image.width;
//                 yolo_input_image->height = resized_image.height;
//                 yolo_input_image->format = resized_image.format;
//                 yolo_input_image->width_stride = resized_image.width_stride;
//                 yolo_input_image->height_stride = resized_image.height_stride;
...
//                     else
//                         free(yolo_input_image->virt_addr); // Drop frame
//                 }
//             } else {
...
//             // --- Run Behavior Analysis & KSS ---
//             detectedObjects.clear();
//             if (cropped_face_results.count > 0 && cropped_face_results.faces[0].face_landmarks_valid) {
//                 face_object_t *face = &cropped_face_results.faces[0];
//                 std::vector<cv::Point> faceLandmarksCv = convert_landmarks_to_cvpoint(face->face_landmarks, NUM_FACE_LANDMARKS);
//                 headPoseTracker.run(faceLandmarksCv);
//                 blinkDetector.run(faceLandmarksCv, resized_image.width, resized_image.height);
//                 yawnMetrics = yawnDetector.run(faceLandmarksCv, resized_image.width, resized_image.height);

//                 // Draw Landmarks for all detected faces (after calibration)
//                 for (int face_idx = 0; face_idx < face_results.count; ++face_idx) {
//                     face_object_t *current_display_face = &face_results.faces[face_idx];
//                     if (current_display_face->face_landmarks_valid) {
//                         // Adjust bounding box to be centered on the face
//                         int box_width = current_display_face->box.right - current_display_face->box.left;
//                         int box_height = current_display_face->box.bottom - current_display_face->box.top;
//                         float center_x = (current_display_face->box.left + current_display_face->box.right) / 2.0f;
//                         float center_y = (current_display_face->box.top + current_display_face->box.bottom) / 2.0f;

//                         // Expand the bounding box symmetrically (e.g., 1.2x width and height)
//                         int new_width = static_cast<int>(box_width * 1.2);
//                         int new_height = static_cast<int>(box_height * 1.2);
//                         int new_left = static_cast<int>(center_x - new_width / 2);
//                         int new_right = static_cast<int>(center_x + new_width / 2);
//                         int new_top = static_cast<int>(center_y - new_height / 2);
//                         int new_bottom = static_cast<int>(center_y + new_height / 2);
...
//                         // Draw adjusted bounding box (Green for consistency)
//                         draw_rectangle(&src_image, new_left, new_top,
//                                        new_right - new_left,
//                                        new_bottom - new_top,
//                                        COLOR_GREEN, 2);
...
//                         if (yolo_results.results[j].prop > 0.4)
...
//                         // Optionally draw the EAR value too
//                         text_stream.str("");
//                         text_stream << "L:" << std::fixed << std::setprecision(2) << blinkDetector.getLeftEARValue();
//                         draw_text(&src_image, text_stream.str().c_str(), left_anchor.x - 30, left_anchor.y - 10, COLOR_WHITE, 10);
//                     }
...
//                         // Optionally draw the EAR value too
//                         text_stream.str("");
//                         text_stream << "R:" << std::fixed << std::setprecision(2) << blinkDetector.getRightEARValue();
//                         draw_text(&src_image, text_stream.str().c_str(), right_anchor.x - 30, right_anchor.y - 10, COLOR_WHITE, 10);
//                     }
//                 }
//             } else { // No face detected after calibration
//                 kssStatus = "Face Not Detected";
//                 extractedTotalKSS = 1;
//                 yawnMetrics = {};
//                 headPoseResults = {};
//             }
...
//             // --- Drawing (Status Text Only) ---
//             if (kssStatus.empty()) status_color_uint = COLOR_GREEN;
//             else if (extractedTotalKSS <= 3) status_color_uint = COLOR_GREEN;
//             else if (extractedTotalKSS <= 7) status_color_uint = COLOR_BLUE;
//             else status_color_uint = COLOR_RED;
//             text_y = 10;
//             if (!kssStatus.empty()) {
...
//             text_y += line_height;
//             text_stream.str("");
//             text_stream << "Yawn Freq: " << static_cast<int>(yawnMetrics.yawnFrequency_5min);
//             draw_text(&src_image, text_stream.str().c_str(), 10, text_y, COLOR_WHITE, text_size);
//             text_y += line_height;
...
//                     if (j < detectedObjects.size() - 1) detected_objects_text += ", ";
//                 }
...
//             // Draw KSS Breakdown
//             text_stream.str("");
//             text_stream << "Perclos KSS: " << perclosKSS;
//             draw_text(&src_image, text_stream.str().c_str(), 10, text_y, COLOR_GREEN, text_size);
//             text_y += line_height;
//             text_stream.str("");
//             text_stream << "Head-pose KSS: " << headposeKSS;
//             draw_text(&src_image, text_stream.str().c_str(), 10, text_y, COLOR_GREEN, text_size);
//             text_y += line_height;
//             text_stream.str("");
//             text_stream << "Yawn KSS: " << yawnKSS;
//             draw_text(&src_image, text_stream.str().c_str(), 10, text_y, COLOR_GREEN, text_size);
//             text_y += line_height;
//             text_stream.str("");
//             text_stream << "Object detection KSS: " << objdectdetectionKSS;
//             draw_text(&src_image, text_stream.str().c_str(), 10, text_y, COLOR_GREEN, text_size);
//             text_y += line_height;
//             text_stream.str("");
...
//         // 1. Draw Fixed ROI (Yellow)
//         draw_rectangle(&src_image, roi_left, roi_top, roi_width, roi_height, COLOR_YELLOW, 2);
//         // 2. Draw Front Passenger's Face Bounding Box (Green) if detected
//         if (passenger_face_idx >= 0) {
//             face_object_t* passenger_face = &face_results.faces[passenger_face_idx];
//             // Adjust bounding box for the front passenger
//             int box_width = passenger_face->box.right - passenger_face->box.left;
//             int box_height = passenger_face->box.bottom - passenger_face->box.top;
//             float center_x = (passenger_face->box.left + passenger_face->box.right) / 2.0f;
//             float center_y = (passenger_face->box.top + passenger_face->box.bottom) / 2.0f;
//             int new_width = static_cast<int>(box_width * 1.2);
//             int new_height = static_cast<int>(box_height * 1.2);
//             int new_left = static_cast<int>(center_x - new_width / 2);
//             int new_right = static_cast<int>(center_x + new_width / 2);
...
//                            COLOR_GREEN, 2);
//         }
//         // 3. Draw Cropped Region (Blue) if used
//         if (use_cropped_region) {
//             draw_rectangle(&src_image, cropped_left, cropped_top,
//                            cropped_right - cropped_left,
...
//     if (yolo_worker_thread.joinable()) yolo_worker_thread.join();
