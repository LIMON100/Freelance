...
int post_process(yolo11_app_context_t *app_ctx,
                 void *outputs, // Can be rknn_output* or rknn_tensor_mem**
                 ...
                 object_detect_result_list *od_results)
{
    ...
    // Determine dfl_len based on RKNPU version/output dims
#ifdef RKNPU1
    int dfl_len = app_ctx->output_attrs[0].dims[2] / 4;
#elif defined(RV1106_1103) // Check RV1106_1103 specifically if its dim order is different
     // Assuming RV1106 output dims are [N, H, W, C] or similar where C has box info
     // Adjust based on actual queried dimensions if needed.
     // Example assuming C is the last dim:
    int dfl_len = app_ctx->output_attrs[0].dims[app_ctx->output_attrs[0].n_dims - 1] / 4;
#else // RKNPU2 (excluding RV1106 specific handling)
    // Assuming RKNPU2 output dims are [N, C, H, W] or similar where C has box info
     // Adjust based on actual queried dimensions if needed.
     // Example assuming C is the second dim:
    int dfl_len = app_ctx->output_attrs[0].dims[1] / 4;
#endif

    int output_per_branch = app_ctx->io_num.n_output / 3; // Assuming 3 branches

    for (int i = 0; i < 3; i++) // Loop through branches
    {
        void* box_buf = nullptr;
        void* score_buf = nullptr;
        void* score_sum_buf = nullptr; // Optional score sum buffer
        ...
        int32_t score_zp = app_ctx->output_attrs[score_idx].zp;
        float score_scale = app_ctx->output_attrs[score_idx].scale;
        int32_t score_sum_zp = (score_sum_idx >= 0) ? app_ctx->output_attrs[score_sum_idx].zp : 0;
        float score_sum_scale = (score_sum_idx >= 0) ? app_ctx->output_attrs[score_sum_idx].scale : 1.0f;

        // Determine grid dimensions based on platform
#ifdef RKNPU1
        grid_h = app_ctx->output_attrs[box_idx].dims[1];
        grid_w = app_ctx->output_attrs[box_idx].dims[0]; // Check RKNPU1 dim order
#elif defined(RV1106_1103)
        grid_h = app_ctx->output_attrs[box_idx].dims[1]; // Assuming NHWC-like for RV1106 output
        grid_w = app_ctx->output_attrs[box_idx].dims[2];
#else // RKNPU2 (generic)
        grid_h = app_ctx->output_attrs[box_idx].dims[2]; // Assuming NCHW-like for RKNPU2 output
        grid_w = app_ctx->output_attrs[box_idx].dims[3];
#endif
        stride = model_in_h / grid_h; // Calculate stride

        // Get buffer pointers based on whether outputs is rknn_output* or rknn_tensor_mem**
#if defined(ZERO_COPY) || defined(RV1106_1103)
        rknn_tensor_mem **_outputs_mem = (rknn_tensor_mem **)outputs;
        if (!_outputs_mem[box_idx] || !_outputs_mem[score_idx]) { /* error */ return -1; }
        box_buf = _outputs_mem[box_idx]->virt_addr;
        score_buf = _outputs_mem[score_idx]->virt_addr;
        if (score_sum_idx >= 0 && _outputs_mem[score_sum_idx]) {
            score_sum_buf = _outputs_mem[score_sum_idx]->virt_addr;
        }
         if (!box_buf || !score_buf || (score_sum_idx >= 0 && !score_sum_buf)) {
            printf("ERROR: YOLO output memory virt_addr is NULL in post_process.\n"); return -1;
        }
#else
        rknn_output *_outputs_struct = (rknn_output *)outputs;
        box_buf = _outputs_struct[box_idx].buf;
        score_buf = _outputs_struct[score_idx].buf;
        if (score_sum_idx >= 0) {
            score_sum_buf = _outputs_struct[score_sum_idx].buf;
        }
         if (!box_buf || !score_buf || (score_sum_idx >= 0 && !score_sum_buf)) {
             printf("ERROR: YOLO output buffer is NULL in post_process.\n"); return -1;
        }
#endif
        ...
        if (app_ctx->is_quant) {
#if defined(RV1106_1103)
            validCount += process_i8_rv1106((int8_t *)box_buf, box_zp, box_scale,
                                             (int8_t *)score_buf, score_zp, score_scale,
                                             (int8_t *)score_sum_buf, score_sum_zp, score_sum_scale,
                                             grid_h, grid_w, stride, dfl_len,
                                             filterBoxes, objProbs, classId, conf_threshold);
#elif defined(RKNPU1) // Handle RKNPU1 case (might be UINT8)
             if (app_ctx->output_attrs[score_idx].type == RKNN_TENSOR_UINT8) {
                 validCount += process_u8((uint8_t *)box_buf, box_zp, box_scale,
                                          (uint8_t *)score_buf, score_zp, score_scale,
                                          (uint8_t *)score_sum_buf, score_sum_zp, score_sum_scale,
                                          grid_h, grid_w, stride, dfl_len,
                                          filterBoxes, objProbs, classId, conf_threshold);
             } else { // Assume INT8 for other RKNPU1 cases if not UINT8
                 validCount += process_i8((int8_t *)box_buf, box_zp, box_scale,
                                          (int8_t *)score_buf, score_zp, score_scale,
                                          (int8_t *)score_sum_buf, score_sum_zp, score_sum_scale,
                                          grid_h, grid_w, stride, dfl_len,
                                          filterBoxes, objProbs, classId, conf_threshold);
             }
#else // RKNPU2 (generic INT8)
            validCount += process_i8((int8_t *)box_buf, box_zp, box_scale,
                                     (int8_t *)score_buf, score_zp, score_scale,
                                     (int8_t *)score_sum_buf, score_sum_zp, score_sum_scale,
                                     grid_h, grid_w, stride, dfl_len,
                                     filterBoxes, objProbs, classId, conf_threshold);
#endif
        } else { // FP32/FP16 (assuming want_float=1 was used in get_outputs or data is already float)
            validCount += process_fp32((float *)box_buf, (float *)score_buf, (float *)score_sum_buf,
                                       grid_h, grid_w, stride, dfl_len,
                                       filterBoxes, objProbs, classId, conf_threshold);
        }
    } // End branch loop

    // --- NMS and Final Result Generation ---
    if (validCount <= 0) { return 0; } // No detections above threshold
    ...
    for (int i = 0; i < validCount; ++i) {
        ...
        // --- Box coordinates are relative to the 640x640 input ---
        // float x1_yolo = filterBoxes[n * 4 + 0];
        // float y1_yolo = filterBoxes[n * 4 + 1];
        // float w_yolo = filterBoxes[n * 4 + 2];
        // float h_yolo = filterBoxes[n * 4 + 3];

        // float x2_yolo = x1_yolo + w_yolo;
        // float y2_yolo = y1_yolo + h_yolo;

        // // --- Scale back to original image coordinates using provided scale/offset ---
        // int x1_orig = roundf(offset_x + x1_yolo * scale_w);
        // int y1_orig = roundf(offset_y + y1_yolo * scale_h);
        // int x2_orig = roundf(offset_x + x2_yolo * scale_w);
        // int y2_orig = roundf(offset_y + y2_yolo * scale_h);

        // // Clamp to original image dimensions (need original image dimensions)
        // // We don't have orig_w/h here. Clamping should ideally happen in the drawing stage in main.cc
        // // or pass orig_w/h to this function. For now, store the calculated original coords.

        // od_results->results[last_count].box.left   = x1_orig;
        // od_results->results[last_count].box.top    = y1_orig;
        // od_results->results[last_count].box.right  = x2_orig;
        // od_results->results[last_count].box.bottom = y2_orig;
        // od_results->results[last_count].prop       = objProbs[i];
        // od_results->results[last_count].cls_id     = classId[n];

        float cx_yolo = filterBoxes[n * 4 + 0];
        float cy_yolo = filterBoxes[n * 4 + 1];
        float w_yolo  = filterBoxes[n * 4 + 2];
        float h_yolo  = filterBoxes[n * 4 + 3];

        // Calculate top-left based on center
        float x1_yolo = cx_yolo - w_yolo / 2.0f;
        float y1_yolo = cy_yolo - h_yolo / 2.0f;
        // float x2_yolo = cx_yolo + w_yolo / 2.0f; // Not needed directly for scaling TLBR
        // float y2_yolo = cy_yolo + h_yolo / 2.0f;

        // Scale top-left and dimensions back
        int x1_orig = roundf(offset_x + x1_yolo * scale_w);
        int y1_orig = roundf(offset_y + y1_yolo * scale_h);
        int w_orig  = roundf(w_yolo * scale_w); // Scale width
        int h_orig  = roundf(h_yolo * scale_h); // Scale height

        // Calculate right and bottom
        int x2_orig = x1_orig + w_orig;
        int y2_orig = y1_orig + h_orig;
