...
std::vector<BYTETracker::STrackPtr> BYTETracker::update(const std::vector<Object>& objects, const cv::Mat& affine_matrix)
{
...
void BYTETracker::motion_compensation(std::vector<STrackPtr>& tracks, const cv::Mat& affine_matrix)
{
    if (affine_matrix.empty() || tracks.empty()) {
        return;
    }

    for (auto& track : tracks) {
        Rect<float>& rect = track->getRect();
        
        std::vector<cv::Point2f> corners(4);
        corners[0] = cv::Point2f(rect.tl_x(), rect.tl_y());
        corners[1] = cv::Point2f(rect.br_x(), rect.tl_y());
        corners[2] = cv::Point2f(rect.br_x(), rect.br_y());
        corners[3] = cv::Point2f(rect.tl_x(), rect.br_y());

        std::vector<cv::Point2f> transformed_corners;
        cv::transform(corners, transformed_corners, affine_matrix);

        float min_x = transformed_corners[0].x;
        float max_x = transformed_corners[0].x;
        float min_y = transformed_corners[0].y;
        float max_y = transformed_corners[0].y;

        for (size_t i = 1; i < transformed_corners.size(); ++i) {
            min_x = std::min(min_x, transformed_corners[i].x);
            max_x = std::max(max_x, transformed_corners[i].x);
            min_y = std::min(min_y, transformed_corners[i].y);
            max_y = std::max(max_y, transformed_corners[i].y);
        }
