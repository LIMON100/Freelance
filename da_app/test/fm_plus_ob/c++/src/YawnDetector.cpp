...
//     // Use const cv::Mat& frame (Readonly)
...
//         yawn_kss = 7;
//     } else if (yawn_duration > 2 || yawn_frequency > 7) {
//         yawn_kss = 9;
//     }
//     return yawn_kss;
// }
...
//                 if (yawn_duration >= MIN_YAWN_DURATION) {

//                     if ((current_time - last_yawn_time) > YAWN_COOLDOWN) {
//                         yawns.push_back({{"start", yawn_start_time}, {"end", current_time}, {"duration", yawn_duration}});
//                         yawn_timestamps.push_back(current_time);
...
YawnDetector::YawnMetrics YawnDetector::run(const cv::Mat& frame, const std::vector<cv::Point>& faceLandmarks) {
    ...
    metrics.isYawning = is_yawn;
    metrics.yawnCount = yawn_frequency;
    metrics.yawnDuration = last_yawn_duration; // Change from yawn_duration to last_yawn_duration
    metrics.yawnFrequency = yawn_frequency;
    metrics.yawnKSS = yawn_kss_;
    return metrics;
}