...
import { v4 as uuidv4 } from "uuid";
import { tool } from "@langchain/core/tools";
import { upsertMemoryInDb } from "../db/db";
...
async function upsertMemory(opts: {
    ...
    await upsertMemoryInDb(userId, memId, content, context)
...
const upsertMemoryTool = tool(upsertMemory, {
    name: "upsertMemory",
    description:
    "Upsert a memory in the database related to the user's preferred gaming platform. If an existing platform preference memory is found, update it by passing the same memoryId, rather than creating a new entry. If the user changes or corrects their platform preference, simply update the existing memory with the new information.",
    schema: z.object({
        content: z.string().describe(
        "The user's preferred gaming platform in a single word or short phrase. For example: 'playstation', 'xbox', or 'pc'."
        ),
        context: z.string().describe(
        "Additional context for why or when the user stated this preference. For example: 'User mentioned this after being asked about platform preference.'"
        ),
        memoryId: z
        .string()
        .optional()
        .describe(
        "The memory ID to overwrite if updating an existing preference. For example, using 'platform_preference' ensures future updates overwrite the same memory."
        ),
    }),
});
