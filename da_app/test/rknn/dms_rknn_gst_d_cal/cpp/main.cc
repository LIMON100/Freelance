...
int main(int argc, char **argv) {
    ...
    const char *video_source = "filesrc location=../../model/wingbody3.mkv ! decodebin ! queue ! videoconvert ! video/x-raw,format=BGR ! appsink name=sink sync=false";
    ...
    while (true) {
        ...
        if (!calibration_done) {
            ...
            text_stream.str(""); text_stream << "Last Blink Duration: " << std::fixed << std::setprecision(2) << blinkDetector.getLastBlinkDuration() << "s";
            draw_text(&src_image, text_stream.str().c_str(), 10, text_y, COLOR_WHITE, text_size); text_y += line_height;
