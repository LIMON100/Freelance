...
class HeadPoseTracker:
    ...
    def run(self, results):
        ...
        if results.multi_face_landmarks:
            ...
            # head_direction = f"{horizontal_direction}, {vertical_direction}, {tilt_direction}"

            # Optional: Simplify if facing forward in all aspects
            if horizontal_direction == vertical_direction == "Facing Forward" and tilt_direction == "No Tilt":
                head_direction = "Facing Forward"

            rows = [
                # ["Yaw", f"{yaw:.1f} deg"],
                # ["Pitch", f"{pitch:.1f} deg"],
                # ["Roll", f"{roll:.1f} deg"],
                ["Head H", f"{horizontal_direction}"],
                ["Head V", f"{vertical_direction}"],
            ]
        return rows