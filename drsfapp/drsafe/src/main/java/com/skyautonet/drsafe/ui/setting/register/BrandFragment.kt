...
class BrandFragment : BaseFragment() {
    ...
    private val selectableAdapter = SelectableAdapter { brand ->
        selectedBrand = brand
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        selectableAdapter.submitBrandList(resources.getStringArray(R.array.brands).toList())
        binding.btnConfirm.setOnClickListener {
            if (selectedBrand.isEmpty()) {
                Toast.makeText(requireContext(), getString(R.string.please_select_a_brand), Toast.LENGTH_SHORT).show()
            } else {
                onBackIconPressed()
            }
        }
