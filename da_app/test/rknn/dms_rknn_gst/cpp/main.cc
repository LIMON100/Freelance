...
// #endif
// // ... (include all other COLOR_ definitions as before) ...
// #ifndef COLOR_YELLOW
...
//         if (deg_pos != std::string::npos) numeric_part = numeric_part.substr(0, deg_pos);
...
//         printf("WARN: Exception parsing head pose value '%s': %s\n", s.c_str(), e.what()); return 0.0;
//     } catch (...) {
//          printf("WARN: Unknown exception parsing head pose value '%s'.\n", s.c_str()); return 0.0;
//     }
// }
// // --- End Helper Functions ---

// // --- YOLO Worker Thread (as before) ---
...
// void yolo_worker_thread_func(yolo11_app_context_t* yolo_ctx_ptr) { /* ... (Implementation as before, including the free()) ... */
//     while (!stop_yolo_worker.load()) {
//         YoloInputData input_data; bool got_data = false;
//         { std::unique_lock<std::mutex> lock(yolo_input_mutex); if (!yolo_input_queue.empty()) { input_data = yolo_input_queue.front(); yolo_input_queue.pop(); got_data = true; } }
//         if (got_data && input_data.image) {
//             YoloOutputData output_data; output_data.frame_id = input_data.frame_id; memset(&output_data.results, 0, sizeof(output_data.results));
//             int ret = inference_yolo11(yolo_ctx_ptr, input_data.image.get(), &output_data.results);
//             if (ret != 0) printf("WARN: YOLO Worker inference failed (frame %ld), ret=%d\n", input_data.frame_id, ret);
//             { std::unique_lock<std::mutex> lock(yolo_output_mutex); if (yolo_output_queue.size() >= MAX_QUEUE_SIZE) yolo_output_queue.pop(); yolo_output_queue.push(output_data); }
//             if (input_data.image->virt_addr) { free(input_data.image->virt_addr); } // Free the copied buffer
//         } else { std::this_thread::sleep_for(std::chrono::milliseconds(5)); }
//     } printf("YOLO Worker Thread Exiting.\n");
// }
// // --- End YOLO Worker Thread ---

// // --- GStreamer Saving Pipeline Setup (as before) ---
// void setupPipeline() { /* (Implementation as before) */
...
// }
// void pushFrameToPipeline(unsigned char* data, int size, int width, int height, GstClockTime duration) { /* (Implementation as before) */
...
// // --- Resource Monitoring Functions (as before) ---
// void calculateOverallFPS(double frame_duration_ms, std::deque<double>& times_deque, double& fps_variable, int max_records) { /* (Implementation as before) */
...
// }
// void getCPUUsage(double& cpu_usage_variable, long& prev_idle, long& prev_total) { /* (Implementation as before) */
...
// }
// void getTemperature(double& temp_variable) { /* (Implementation as before) */
...
//     // --- Resource Monitoring Vars (as before) ---
...
//     // GStreamer input pipeline string (as before)
...
//     std::chrono::steady_clock::time_point calibration_start_time; // <<< Use steady_clock
...
//     // --- Init GStreamer input (as before) ---
...
//         auto frame_start_time = std::chrono::high_resolution_clock::now(); // Can use high_res here for overall FPS
//         current_frame_id++;

//         // Wrap GStreamer Data (as before)
...
//                     calibration_start_time = std::chrono::steady_clock::now(); // <<< Use steady_clock
//                     calibration_timer_started = true;
//                     printf("INFO: Calibration timer started.\n");
//                 }

//                 // Run HeadPose tracker
//                 std::vector<cv::Point> calib_faceLandmarks = convert_landmarks_to_cvpoint(face_results.faces[0].face_landmarks, NUM_FACE_LANDMARKS);
//                 headPoseTracker.run(calib_faceLandmarks);
//                 bool head_pose_calibrated = headPoseTracker.isCalibrated();

//                 // Check eye landmark consistency
//                 bool eyes_valid_this_frame = face_results.faces[0].eye_landmarks_left_valid && face_results.faces[0].eye_landmarks_right_valid;
//                 if (eyes_valid_this_frame) consecutive_valid_eyes_frames++; else consecutive_valid_eyes_frames = 0;
//                 bool eyes_consistently_valid = (consecutive_valid_eyes_frames >= REQUIRED_VALID_EYES_FRAMES);

//                 // Check timer
//                 auto now = std::chrono::steady_clock::now(); // <<< Use steady_clock
//                 double elapsed_calib_seconds = std::chrono::duration<double>(now - calibration_start_time).count(); // <<< Use steady_clock difference

//                 if (elapsed_calib_seconds >= 10.0) {
//                     if (head_pose_calibrated && eyes_consistently_valid) {
//                         calibration_done = true; printf("INFO: Calibration Complete!\n");
//                     } else {
//                         calibration_timer_started = false; consecutive_valid_eyes_frames = 0;
//                         printf("WARN: Calibration time expired, criteria not met (HeadPose:%d, EyesConsistent:%d). Retrying.\n", head_pose_calibrated, eyes_consistently_valid);
//                     }
//                 }

//                 // --- Drawing During Calibration ---
//                 for (int i = 0; i < face_results.count; ++i) { draw_rectangle(&src_image, face_results.faces[i].box.left, face_results.faces[i].box.top, face_results.faces[i].box.right - face_results.faces[i].box.left, face_results.faces[i].box.bottom - face_results.faces[i].box.top, COLOR_YELLOW, 2); }
//                 std::string calib_text = "Calibrating... " + std::to_string(static_cast<int>(elapsed_calib_seconds)) + "s"; if (!head_pose_calibrated) calib_text += " (Head)"; if (!eyes_consistently_valid) calib_text += " (Eyes)"; draw_text(&src_image, calib_text.c_str(), 10, 30, COLOR_YELLOW, status_text_size);

//             } else { // No face detected
//                 calibration_timer_started = false; consecutive_valid_eyes_frames = 0;
//                 draw_text(&src_image, "Calibration: Waiting for face...", 10, 30, COLOR_YELLOW, status_text_size);
//             }

//         } else { // Normal Processing Phase
//             // (Logic as before: Queue YOLO, Get YOLO, Run Behavior, Calculate KSS)
...
//             if (yolo_input_image->virt_addr) { memcpy(yolo_input_image->virt_addr, src_image.virt_addr, src_image.size); /* copy other fields */ yolo_input_image->width = src_image.width; yolo_input_image->height = src_image.height; yolo_input_image->format = src_image.format; yolo_input_image->width_stride = src_image.width_stride; yolo_input_image->height_stride = src_image.height_stride; yolo_input_image->fd = -1; { std::unique_lock<std::mutex> lock(yolo_input_mutex); if (yolo_input_queue.size() < MAX_QUEUE_SIZE) yolo_input_queue.push({current_frame_id, yolo_input_image}); else free(yolo_input_image->virt_addr); } } else { printf("ERROR: Failed alloc YOLO input copy.\n"); }
...
//                 int headPoseKSSValue = 1; if (headPoseResults.rows.size() >= 4) { for (const auto& row : headPoseResults.rows) { if (row.size() >= 2 && row[0] == "Head KSS") { try { headPoseKSSValue = std::stoi(row[1]); } catch (...) { headPoseKSSValue = 1; } break; } } } kssCalculator.setHeadPose(headPoseKSSValue);
...
//             // (Draw other status info as before)
...
//             text_stream.str(""); text_stream << "Blink KSS: " << blinkKSS; draw_text(&src_image, text_stream.str().c_str(), 10, text_y, COLOR_GREEN, text_size); text_y += line_height;
...
//         auto frame_processing_end_time = std::chrono::high_resolution_clock::now(); // Can use high_res here
...
//         std::stringstream info_ss; info_ss << "FPS:" << std::fixed << std::setprecision(1) << overallFPS << "|CPU:" << currentCpuUsage << "%|T:" << currentTemp << "C";
...
//     // GStreamer input pipeline string (as before)
//     const char *video_source = "filesrc location=../../model/yawning.mkv ! decodebin ! queue ! videoconvert ! video/x-raw,format=BGR ! appsink name=sink sync=false";
//     // const char *video_source = "v4l2src device=/dev/video1 ! queue ! videoconvert ! video/x-raw,format=BGR,width=1920,height=1080,framerate=30/1 ! appsink name=sink sync=false";
...
//     // ++ Updated Calibration State Variables ++
...
//     // !! Relaxed Thresholds !!
//     const float EAR_STDDEV_THRESHOLD = 0.04;  // Relaxed from 0.015
//     const double MOUTH_DIST_STDDEV_THRESHOLD = 15.0; // Relaxed from 5.0
//     // !! Adjusted Stable Frames Requirement !!
//     const int REQUIRED_STABLE_FRAMES = CALIB_WINDOW_SIZE + 5; // Require window size + 5 stable frames
...
//         // --- Calibration Phase Logic ---
//         if (!calibration_done) {
//             bool head_pose_calibrated = false; // Assume not calibrated until checked
//             bool eyes_consistently_valid = false;
//             double elapsed_calib_seconds = 0.0;
//             std::string calib_status_detail = ""; // To build the status string
...
//                     calibration_timer_started = true;
//                     printf("INFO: Calibration timer started.\n");
//                     consecutive_valid_eyes_frames = 0; consecutive_stable_ear_frames = 0; consecutive_stable_mouth_frames = 0;
//                     calib_left_ears.clear(); calib_right_ears.clear(); calib_mouth_dists.clear();
//                     ear_calibrated = false; mouth_calibrated = false;
//                 }

//                 // --- Run Head Pose Tracker ---
//                 std::vector<cv::Point> calib_faceLandmarks = convert_landmarks_to_cvpoint(face_results.faces[0].face_landmarks, NUM_FACE_LANDMARKS);
//                 headPoseTracker.run(calib_faceLandmarks);
//                 head_pose_calibrated = headPoseTracker.isCalibrated();
//                 if (!head_pose_calibrated) calib_status_detail += " (Head)";

//                 // --- Check Eye Landmark Consistency ---
//                 bool eyes_valid_this_frame = face_results.faces[0].eye_landmarks_left_valid && face_results.faces[0].eye_landmarks_right_valid;
//                 if (eyes_valid_this_frame) consecutive_valid_eyes_frames++; else consecutive_valid_eyes_frames = 0;
//                 eyes_consistently_valid = (consecutive_valid_eyes_frames >= REQUIRED_VALID_EYES_FRAMES);
//                 if (!eyes_consistently_valid) calib_status_detail += " (Eyes Valid)";

//                 // --- Check EAR Stability ---
//                 if (!ear_calibrated && eyes_valid_this_frame) { // Only check if eyes are valid
//                     const std::vector<int> LEFT_EYE_POINTS = { 33, 160, 158, 133, 153, 144 };
//                     const std::vector<int> RIGHT_EYE_POINTS = { 362, 385, 387, 263, 380, 373 };
//                     float current_left_ear = calculate_ear_simple(calib_faceLandmarks, LEFT_EYE_POINTS);
//                     float current_right_ear = calculate_ear_simple(calib_faceLandmarks, RIGHT_EYE_POINTS);
//                     calib_left_ears.push_back(current_left_ear); calib_right_ears.push_back(current_right_ear);
//                     if (calib_left_ears.size() > CALIB_WINDOW_SIZE) calib_left_ears.pop_front();
//                     if (calib_right_ears.size() > CALIB_WINDOW_SIZE) calib_right_ears.pop_front();

//                     if (calib_left_ears.size() >= CALIB_WINDOW_SIZE) {
//                         float left_stddev = calculate_stddev(calib_left_ears);
//                         float right_stddev = calculate_stddev(calib_right_ears);
//                         // --- Debug Print ---
//                         // printf("Calib EAR L: %.3f (Std: %.4f), R: %.3f (Std: %.4f), StableFrames: %d\n", current_left_ear, left_stddev, current_right_ear, right_stddev, consecutive_stable_ear_frames);
//                         if (left_stddev < EAR_STDDEV_THRESHOLD && right_stddev < EAR_STDDEV_THRESHOLD) {
//                             consecutive_stable_ear_frames++;
//                         } else {
//                             consecutive_stable_ear_frames = 0;
//                         }
//                     } else consecutive_stable_ear_frames = 0;
//                     ear_calibrated = (consecutive_stable_ear_frames >= REQUIRED_STABLE_FRAMES);
//                 } else if (!eyes_valid_this_frame) { // Reset if eyes become invalid
//                      consecutive_stable_ear_frames = 0; ear_calibrated = false; calib_left_ears.clear(); calib_right_ears.clear();
//                 }
//                 if (!ear_calibrated) calib_status_detail += " (EAR Stable)";

//                  // --- Check Mouth Distance stability ---
//                 if (!mouth_calibrated) {
//                     double current_mouth_dist = calculate_mouth_dist_simple(calib_faceLandmarks);
//                     calib_mouth_dists.push_back(current_mouth_dist);
//                     if (calib_mouth_dists.size() > CALIB_WINDOW_SIZE) calib_mouth_dists.pop_front();

//                     if (calib_mouth_dists.size() >= CALIB_WINDOW_SIZE) {
//                         double mouth_stddev = calculate_stddev(calib_mouth_dists);
//                          // --- Debug Print ---
//                         // printf("Calib Mouth Dist: %.1f (Std: %.2f), StableFrames: %d\n", current_mouth_dist, mouth_stddev, consecutive_stable_mouth_frames);
//                         if (mouth_stddev < MOUTH_DIST_STDDEV_THRESHOLD) {
//                             consecutive_stable_mouth_frames++;
//                         } else {
//                             consecutive_stable_mouth_frames = 0;
//                         }
//                     } else consecutive_stable_mouth_frames = 0;
//                     mouth_calibrated = (consecutive_stable_mouth_frames >= REQUIRED_STABLE_FRAMES);
//                 }
//                 if (!mouth_calibrated) calib_status_detail += " (Mouth Stable)";
...
//                         calibration_done = true; printf("INFO: Calibration Complete! (Head, Eyes Valid, EAR Stable, Mouth Stable)\n");
//                     } else {
//                         calibration_timer_started = false; consecutive_valid_eyes_frames = 0; consecutive_stable_ear_frames = 0; consecutive_stable_mouth_frames = 0;
//                         ear_calibrated = false; mouth_calibrated = false; calib_left_ears.clear(); calib_right_ears.clear(); calib_mouth_dists.clear();
//                         printf("WARN: Calibration time expired, criteria not met (Head:%d, EyesValid:%d, EARStable:%d, MouthStable:%d). Retrying.\n", head_pose_calibrated, eyes_consistently_valid, ear_calibrated, mouth_calibrated);
//                     }
//                 }

//                 // --- Drawing During Calibration ---
//                 for (int i = 0; i < face_results.count; ++i) { draw_rectangle(&src_image, face_results.faces[i].box.left, face_results.faces[i].box.top, face_results.faces[i].box.right - face_results.faces[i].box.left, face_results.faces[i].box.bottom - face_results.faces[i].box.top, COLOR_YELLOW, 2); }
//                 std::string calib_text = "Calibrating... " + std::to_string(static_cast<int>(elapsed_calib_seconds)) + "s" + calib_status_detail;
//                 draw_text(&src_image, calib_text.c_str(), 10, 30, COLOR_YELLOW, status_text_size);

//             } else { // No face detected
//                 // Reset timer and ALL counters/flags if face is lost
//                 calibration_timer_started = false; consecutive_valid_eyes_frames = 0; consecutive_stable_ear_frames = 0; consecutive_stable_mouth_frames = 0;
//                 ear_calibrated = false; mouth_calibrated = false; calib_left_ears.clear(); calib_right_ears.clear(); calib_mouth_dists.clear();
//                 draw_text(&src_image, "Calibration: Waiting for face...", 10, 30, COLOR_YELLOW, status_text_size);
//             }
...
//         } else { // Normal Processing Phase
//             // (Logic remains the same: Queue YOLO, Get YOLO, Run Behavior, Calculate KSS)
//             auto yolo_input_image = std::make_shared<image_buffer_t>(); yolo_input_image->size = src_image.size; yolo_input_image->virt_addr = (unsigned char*)malloc(yolo_input_image->size); if (yolo_input_image->virt_addr) { memcpy(yolo_input_image->virt_addr, src_image.virt_addr, src_image.size); /* copy other fields */ yolo_input_image->width = src_image.width; yolo_input_image->height = src_image.height; yolo_input_image->format = src_image.format; yolo_input_image->width_stride = src_image.width_stride; yolo_input_image->height_stride = src_image.height_stride; yolo_input_image->fd = -1; { std::unique_lock<std::mutex> lock(yolo_input_mutex); if (yolo_input_queue.size() < MAX_QUEUE_SIZE) yolo_input_queue.push({current_frame_id, yolo_input_image}); else free(yolo_input_image->virt_addr); } } else { printf("ERROR: Failed alloc YOLO input copy.\n"); }
//             bool yolo_result_received = false; { std::unique_lock<std::mutex> lock(yolo_output_mutex); if (!yolo_output_queue.empty()) { YoloOutputData data = yolo_output_queue.front(); yolo_output_queue.pop(); yolo_results = data.results; yolo_result_received = true; } }
//             detectedObjects.clear();
//             if (face_results.count > 0 && face_results.faces[0].face_landmarks_valid) {
//                 face_object_t *face = &face_results.faces[0]; std::vector<cv::Point> faceLandmarksCv = convert_landmarks_to_cvpoint(face->face_landmarks, NUM_FACE_LANDMARKS); headPoseResults = headPoseTracker.run(faceLandmarksCv); blinkDetector.run(faceLandmarksCv, src_image.width, src_image.height); yawnMetrics = yawnDetector.run(faceLandmarksCv, src_image.width, src_image.height);
//                 kssCalculator.setPerclos(blinkDetector.getPerclos()); int headPoseKSSValue = 1; if (headPoseResults.rows.size() >= 4) { for (const auto& row : headPoseResults.rows) { if (row.size() >= 2 && row[0] == "Head KSS") { try { headPoseKSSValue = std::stoi(row[1]); } catch (...) { headPoseKSSValue = 1; } break; } } } kssCalculator.setHeadPose(headPoseKSSValue); kssCalculator.setYawnMetrics(yawnMetrics.isYawning, yawnMetrics.yawnFrequency, yawnMetrics.yawnDuration); kssCalculator.setBlinksLastMinute(blinkDetector.getBlinksInWindow()); if (yolo_results.count > 0) { for (int j = 0; j < yolo_results.count; ++j) { if (yolo_results.results[j].prop > 0.4) detectedObjects.push_back(coco_cls_to_name(yolo_results.results[j].cls_id)); } } double now_seconds = std::chrono::duration<double>(std::chrono::system_clock::now().time_since_epoch()).count(); kssCalculator.setDetectedObjects(detectedObjects, now_seconds);
//                 auto kssBreakdownResults = kssCalculator.calculateCompositeKSS(); if (kssBreakdownResults.size() > 5 && kssBreakdownResults[5].size() == 2) { try { perclosKSS = std::stoi(kssBreakdownResults[0][1]); blinkKSS = std::stoi(kssBreakdownResults[1][1]); headposeKSS = std::stoi(kssBreakdownResults[2][1]); yawnKSS = std::stoi(kssBreakdownResults[3][1]); objdectdetectionKSS = std::stoi(kssBreakdownResults[4][1]); extractedTotalKSS = std::stoi(kssBreakdownResults[5][1]); } catch (...) { extractedTotalKSS = 1; } } else { extractedTotalKSS = 1; } kssStatus = kssCalculator.getKSSAlertStatus(extractedTotalKSS);
//             } else { kssStatus = "Face Not Detected"; extractedTotalKSS = 1; yawnMetrics = {}; headPoseResults = {}; }
...
//             if (headPoseResults.rows.size() >= 3) { std::string headpose_text = "Yaw:" + headPoseResults.rows[0][1] + " Pitch:" + headPoseResults.rows[1][1] + " Roll:" + headPoseResults.rows[2][1]; draw_text(&src_image, headpose_text.c_str(), 10, text_y, COLOR_WHITE, text_size); text_y += line_height; } else { draw_text(&src_image, "Head Pose: N/A", 10, text_y, COLOR_WHITE, text_size); text_y += line_height; } text_stream.str(""); text_stream << "Yawning: " << (yawnMetrics.isYawning ? "Yes" : "No"); draw_text(&src_image, text_stream.str().c_str(), 10, text_y, COLOR_WHITE, text_size); text_y += line_height; 
...
//             std::string detected_objects_text = ""; if (!detectedObjects.empty()) { detected_objects_text = "Detected: "; for (size_t j = 0; j < detectedObjects.size(); ++j) { detected_objects_text += detectedObjects[j]; if (j < detectedObjects.size() - 1) detected_objects_text += ", "; } draw_text(&src_image, detected_objects_text.c_str(), 10, text_y, COLOR_ORANGE, text_size); text_y += line_height; } text_stream.str(""); 
            
//             // text_stream << "Blink KSS: " << blinkKSS; draw_text(&src_image, text_stream.str().c_str(), 10, text_y, COLOR_GREEN, text_size); 
//             // text_y += line_height; 
            
//             text_stream.str(""); 
//             text_stream << "Perclos KSS: " << perclosKSS; draw_text(&src_image, text_stream.str().c_str(), 10, text_y, COLOR_GREEN, text_size); text_y += line_height; text_stream.str(""); text_stream << "Yawn KSS: " << yawnKSS; draw_text(&src_image, text_stream.str().c_str(), 10, text_y, COLOR_GREEN, text_size); text_y += line_height; text_stream.str(""); text_stream << "Head-pose KSS: " << headposeKSS; draw_text(&src_image, text_stream.str().c_str(), 10, text_y, COLOR_GREEN, text_size); text_y += line_height; text_stream.str(""); text_stream << "Object detection KSS: " << objdectdetectionKSS; draw_text(&src_image, text_stream.str().c_str(), 10, text_y, COLOR_GREEN, text_size); text_y += line_height; text_stream.str(""); text_stream << "Composite KSS: " << extractedTotalKSS; draw_text(&src_image, text_stream.str().c_str(), 10, text_y, status_color_uint, text_size); text_y += line_height;
//         } // End if (!calibration_done) / else
...
int main(int argc, char **argv) {
    ...
    // --- Calibration State Variables (as before) ---
    bool calibration_done = false;
    std::chrono::steady_clock::time_point calibration_start_time;
    bool calibration_timer_started = false;
    int consecutive_valid_eyes_frames = 0;
    ...
    while (true) {
        ...
        // --- Calibration Phase Logic (as before) ---
        if (!calibration_done) {
            ...
            text_stream.str(""); text_stream << "Yawning: " << (yawnMetrics.isYawning ? "Yes" : "No"); draw_text(&src_image, text_stream.str().c_str(), 10, text_y, COLOR_WHITE, text_size); text_y += line_height;
            // *** FIXED: Use correct variable name ***
            text_stream.str(""); text_stream << "Yawn Freq (Last 5Min): " << static_cast<int>(yawnMetrics.yawnFrequency_5min); draw_text(&src_image, text_stream.str().c_str(), 10, text_y, COLOR_WHITE, text_size); text_y += line_height;
            std::string detected_objects_text = ""; if (!detectedObjects.empty()) { detected_objects_text = "Detected: "; for (size_t j = 0; j < detectedObjects.size(); ++j) { detected_objects_text += detectedObjects[j]; if (j < detectedObjects.size() - 1) detected_objects_text += ", "; } draw_text(&src_image, detected_objects_text.c_str(), 10, text_y, COLOR_ORANGE, text_size); text_y += line_height; }
            // Draw KSS Breakdown
            text_stream.str(""); text_stream << "Perclos KSS: " << perclosKSS; draw_text(&src_image, text_stream.str().c_str(), 10, text_y, COLOR_GREEN, text_size); text_y += line_height;
