...
int main(int argc, char **argv) {
    ...
    // --- GStreamer input pipeline string ---
    const char *video_source = "filesrc location=../../model/ov6.mp4 ! decodebin ! queue ! videoconvert ! video/x-raw,format=BGR ! appsink name=sink sync=false";
    // const char *video_source = "v4l2src device=/dev/video1 ! queue ! videoconvert ! video/x-raw,format=BGR,width=1920,height=1080,framerate=30/1 ! appsink name=sink sync=false";
    ...
    // --- Calibration State Variables ---
    // ... (Calibration vars remain the same) ...
    ...
    // --- Init GStreamer input ---
    // ... (GStreamer init remains the same) ...
    ...
    while (true) {
        ...
        if (first_frame) {
             ...
             int roi_start_x = (int)(frame_width * 0.40); // Start further in (40% from left)
             int roi_width = (int)(frame_width * 0.55);  // Cover 55% horizontally (40% to 95%)
             driver_identification_zone = cv::Rect(
                 roi_start_x,
                 (int)(frame_height * 0.1),     // Start 10% from top (unchanged)
                 roi_width,
                 (int)(frame_height * 0.8)      // Height = 80% (unchanged)
             );
             // Ensure it stays within bounds after calculation
        ...
        // --- Run Face Analysis ---
        // ... (Face analysis call) ...
        ret = inference_face_analyzer(&app_ctx.face_ctx, &src_image, &face_results); if (ret != 0) { printf("WARN: Face Analyzer Inference failed frame %ld, ret=%d\n", current_frame_id, ret); face_results.count = 0; }
        ...
        if (!calibration_done) {
            ...
            if (driver_tracked_this_frame && current_tracked_driver_idx != -1) {
                ...
                box_rect_t driver_box = driver_face->box; int box_w = driver_box.right - driver_box.left; int box_h = driver_box.bottom - driver_box.top;
                if (box_w > 0 && box_h > 0) {
                    int box_center_x = (driver_box.left + driver_box.right) / 2; int box_center_y = (driver_box.top + driver_box.bottom) / 2;
                    // --- TUNABLE Expansion Factors ---
                    const float width_expansion = 1.65f; // Increase width slightly (5% more than 1.6)
                    const float height_expansion_up = 1.0f;  // More upward coverage (increased by 2-3% total height: 0.8 -> 1.0)
                    const float height_expansion_down = 2.3f;// Slightly increase downward too? (increase by 2-3% total height: 2.2 -> 2.3)
                    // ---------------------------------
                    ...
                    int roi_y = box_center_y - static_cast<int>(box_h * height_expansion_up); // Anchor top
                    driver_object_roi.x = std::max(0, roi_x); driver_object_roi.y = std::max(0, roi_y);
                    driver_object_roi.width = std::min(roi_width, src_image.width - driver_object_roi.x); driver_object_roi.height = std::min(roi_height, src_image.height - driver_object_roi.y);
                ...
                // ... (YOLO filtering logic remains the same) ...
                 if (valid_object_roi && yolo_results.count > 0) { for (int j = 0; j < yolo_results.count; ++j) { object_detect_result* det = &yolo_results.results[j]; cv::Point obj_center((det->box.left + det->box.right) / 2, (det->box.top + det->box.bottom) / 2); if (driver_object_roi.contains(obj_center) && det->prop > 0.4) { driver_detectedObjects.push_back(coco_cls_to_name(det->cls_id)); } } } detectedObjects = driver_detectedObjects;


                // --- Run Behavior Modules on Driver ---
                // ... (Behavior module calls and KSS input setting remain the same) ...
                ...
                // ... (KSS calculation remains the same) ...
        ...
        if (!first_frame) {
             draw_rectangle(&src_image, driver_identification_zone.x, driver_identification_zone.y,
                            driver_identification_zone.width, driver_identification_zone.height, COLOR_YELLOW, 1);
             draw_text(&src_image, "Driver ID Zone", driver_identification_zone.x + 5, driver_identification_zone.y + 15, COLOR_YELLOW, 10);
        }
        if (valid_object_roi) { // Use the flag calculated in the processing block
            draw_rectangle(&src_image, driver_object_roi.x, driver_object_roi.y,
                           driver_object_roi.width, driver_object_roi.height, COLOR_MAGENTA, 1);
            draw_text(&src_image, "Driver Obj ROI", driver_object_roi.x + 5, driver_object_roi.y + 15, COLOR_MAGENTA, 10);
        }
        #endif

        // --- Draw Resource Monitoring Info (Always Draw) ---
        // ... (Resource drawing remains the same) ...
         auto frame_processing_end_time = std::chrono::high_resolution_clock::now(); double frame_duration_ms = std::chrono::duration<double, std::milli>(frame_processing_end_time - frame_start_time).count(); calculateOverallFPS(frame_duration_ms, frame_times, overallFPS, max_time_records); getCPUUsage(currentCpuUsage, prevIdleTime, prevTotalTime); getTemperature(currentTemp); std::stringstream info_ss; info_ss << "FPS:" << std::fixed << std::setprecision(1) << overallFPS << "|CPU:" << currentCpuUsage << "%|T:" << currentTemp << "C"; draw_text(&src_image, info_ss.str().c_str(), 10, src_image.height - 20, COLOR_WHITE, 10);
